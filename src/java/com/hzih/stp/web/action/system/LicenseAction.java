...
public class LicenseAction extends ActionSupport{
    ...
    public String about() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "平台说明","用户获取平台说明信息成功", 1);
        } catch (Exception e) {
            logger.error("平台说明", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "平台说明","用户获取平台说明信息失败", 0);
        }
    ...
    public String readLocal() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "证书管理","用户获取本地许可证信息成功", 1);
        } catch (Exception e) {
            logger.error("证书管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "证书管理","用户获取本地许可证信息失败", 0);
        }
		base.actionEnd(response, json ,result);
        return null;
    }
    ...
    public String readRemote() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "证书管理","用户获取远程许可证信息成功", 1);
        } catch (Exception e) {
            logger.error("证书管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "证书管理","用户获取远程许可证信息失败", 0);
        }
		base.actionEnd(response, json ,result);
        return null;
    }
    ...
    public String uploadLocal() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "证书管理","用户上传本地许可证信息成功", 1);
        } catch (Exception e) {
            logger.error("证书管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "证书管理","用户上传本地许可证信息失败", 0);
            msg = "上传失败";
        }
...
//                }
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "证书管理","用户上传远程许可证信息成功", 1);
        } catch (Exception e) {
            logger.error("证书管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "证书管理","用户上传远程许可证信息失败", 0);
            msg = "上传失败";
        }
        json = "{'success':true,'msg':'"+msg+"'}";
		base.actionEnd(response, json ,result);
