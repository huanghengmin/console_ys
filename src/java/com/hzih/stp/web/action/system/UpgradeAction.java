...
public class UpgradeAction extends ActionSupport{
    ...
    public String selectWar() throws IOException {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "版本升级","获取服务软件包名列表成功 ", 1);
        } catch (Exception e) {
            logger.error("版本升级", e);
            logService.newManagerLog("ERROR",  SessionUtils.getAccount(request).getUserName(), "版本升级","获取服务软件包名列表不成功 ", 0);
        }
    ...
    public String selectJar() throws IOException {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "版本升级","获取自有Jar软件包列表名成功 ", 1);
        } catch (Exception e) {
            logger.error("版本升级", e);
            logService.newManagerLog("ERROR",  SessionUtils.getAccount(request).getUserName(), "版本升级","获取自有Jar软件包名列表不成功 ", 0);
        }
        actionBase.actionEnd(response, json, result);
        return null;
    }
        ...
        try {
            if(uploadFileFileName.endsWith(".war")){
                ...
                if(isFile){
                    ...
                    msg = "上传成功,点击[确定]返回页面!";
                    logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "版本升级","上传需要更新的文件"+uploadFileFileName+"成功 ", 1);
                } else {
                    msg = "上传的文件是非法的!";
                    logService.newManagerLog("WARN",  SessionUtils.getAccount(request).getUserName(), "版本升级","上传需要更新的文件"+uploadFileFileName+"是非法的 ", 1);
                }
            }else{
                msg = "上传的文件不是[*.war]文件";
            }
            ...
            logger.error("版本升级", e);
            logService.newManagerLog("ERROR",  SessionUtils.getAccount(request).getUserName(), "版本升级","上传需要更新的文件"+uploadFileFileName+"不成功 ", 0);
        ...
        try {
            ...
            if(isOk){
                ...
                if(files.length>0){
                    ...
                    if(msg==null){
                        logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "版本升级","升级成功 ", 1);
                        upgrade("stp");
                        logService.newSysLog("INFO","平台管理","升级","升级成功");
                        msg = "升级成功,点击[确定]返回页面!";
                    }
                }else {
                    logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "版本升级","升级失败,没有需要升级的服务", 0);
                    msg = "没有需要升级的服务";
                }
            } else {
                logService.newManagerLog("WARN",  SessionUtils.getAccount(request).getUserName(), "版本升级","升级失败,升级过于频繁:10分中内多次升级", 0);
                msg = "升级过于频繁,请于"+upgradeTime+"后升级!";
            }
        } catch (Exception e) {
            logger.error("版本升级", e);
            logService.newManagerLog("ERROR",  SessionUtils.getAccount(request).getUserName(), "版本升级","升级不成功 ", 0);
    ...
    public String backup() throws IOException {
        ...
        try {
            ...
            if(msg==null){
                ...
                logService.newSysLog("INFO","平台管理","回退","回退准备完成成功");
                logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "版本升级","回退"+warName+"准备完成 ", 1);
            }
        } catch (Exception e) {
            logger.error("版本升级", e);
            logService.newSysLog("ERROR","平台管理","回退","回退准备失败"+e.getMessage());
            logService.newManagerLog("ERROR",  SessionUtils.getAccount(request).getUserName(), "版本升级","回退准备失败 ", 0);
