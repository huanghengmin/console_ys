    ...
    public String recover() throws Exception {
        ...
        try{
            ...
            if(map!=null && map.size()==0){
                ...
                msg = "配置恢复成功";
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"配置恢复",msg, 1);
                ...
                logger.info(msg);
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"配置恢复","恢复失败,临时表建立失败!", 0);
            } else {
                msg = "配置恢复失败";
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"配置恢复",msg, 0);
            }
        } catch (Exception e){
            logger.error("配置恢复",e);
            msg = "配置恢复失败";
