...
//    private String privated;
    ...
    public String select() throws IOException{
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "平台初始化","用户获取平台通道信息成功", 1);
        } catch (Exception e) {
            logger.error("平台初始化", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "平台初始化","用户获取平台通道信息失败", 0);
    ...
    public String arpA() throws IOException {
        HttpServletRequest request = ServletActionContext.getRequest();
		HttpServletResponse response = ServletActionContext.getResponse();
		ActionBase base = new ActionBase();
		String result =	base.actionBegin(request);
        ...
        try{
            ...
            if(osInfo.isLinux()){
                ...
                Proc proc = new Proc();
                proc.exec(command);
                String arpStr = proc.getOutput();
                logger.info(arpStr);
                String[] arpStrs = arpStr.split("\n");
                for (int i = 0; i < arpStrs.length; i++) {
                    String line = arpStrs[i].trim();
                    msg += line.substring(line.indexOf("(")).trim();
                }
            } else {
                msg = "操作系统不对";
//                msg = "(192.168.1.1) at ec:6c:9f:04:04:6e [ether] on eth44(192.168.1.178) at 40:6c:8f:53:61:c2 [ether] on eth44";
            }
        } catch (Exception e){
            logger.error("测试错误",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"平台初始化","测试失败!", 0);
            msg = "测试失败!";
        }
    ...
    public String send() throws IOException {
        ...
        try{
            ...
            if (serviceResponse.getCode()==200) {
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"平台初始化","发送测试消息成功!", 1);
                msg = "<font color=\"green\">测试发送成功,到内网平台<br>初始化中查看测试结果</font>";
            } else {
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"平台初始化","发送测试消息失败!", 0);
    ...
    public String sendMonitor() throws IOException {
        ...
        try{
            ...
            if (serviceResponse.getCode()==200) {
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"设备运行监控","发送测试消息成功!", 1);
                msg = "<font color=\"green\">测试发送成功,到内网平台<br>设备运行监控中查看测试结果</font>";
            } else {
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"设备运行监控","发送测试消息失败!", 0);
                msg = "<font color=\"red\">测试失败:"+serviceResponse.getCode()+"</font>";
            }
        } catch (Exception e){
...
//            }
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "平台初始化","用户修改平台通道信息成功", 1);
        } catch (Exception e) {
            logger.error("平台初始化", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "平台初始化","用户修改平台通道信息失败", 0);
            msg = "保存失败";
        }
        String json = "{success:true,msg:'"+msg+"'}";
		base.actionEnd(response, json ,result);
