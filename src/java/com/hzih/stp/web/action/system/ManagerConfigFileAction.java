...
//		st.nextToken();
		...
		try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"配置文件管理","下载文件成功!", 1);
        } catch (Exception e){
            logger.error("配置文件管理-下载文件",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"配置文件管理","下载文件失败!", 0);
        }
        ...
        try{
            if(uploadFileFileName.equals("config.xml")){
                ...
                if(isFile) {
                    ...
                    logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(),"配置文件管理",msg, 1);
                }
            } else {
                msg = "文件名必须是config.xml!";
                logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(),"配置文件管理",msg, 0);
            }
        }catch (Exception e){
            logger.error("配置文件管理-上传文件",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"配置文件管理","上传文件失败!", 0);
            msg = "上传失败!";
        }
    ...
    private String uploadXml(HttpServletRequest request) throws IOException {
    	...
    	if(type.equals("inner")){
            ...
            if(files.length>0){
                msg = "上传成功,点击[确定]返回页面!";
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"配置文件管理","上传成功", 1);
            }else{
              	msg = "可信端配置文件上传失败!";
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"配置文件管理","上传失败", 0);
            }
        }else if(type.equals("out")){
