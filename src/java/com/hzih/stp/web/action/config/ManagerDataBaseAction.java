...
public class ManagerDataBaseAction extends ActionSupport {
	...
	public String select() throws Exception {
        ...
        try{
            ...
            logService.newManagerLog("INFO",SessionUtils.getAccount(request).getUserName(),"审计库管理","查找审计库信息成功", 1);
        } catch (Exception e){
            logger.error("审计库管理",e);
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"审计库管理","查找审计库信息失败", 0);
        }
...
//            tempNode.setText(request.getParameter("backup_password"));
            ...
            msg = "保存成功";
            logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                    .getUserName(), "审计库管理", "更新审计库信息成功", 1);
        } catch (Exception e) {
            logger.error("审计库管理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                    .getUserName(), "审计库管理", "更新审计库信息失败", 0);
            msg = "保存失败";
        } finally {
            writer.close();
        }
	...
	public String testBackupDB() throws Exception {
		...
		try {
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                               .getUserName(), "审计库管理", "测试备份审计库信息成功", 1);
		} catch (Exception e) {
			canConnect = "false";
			logger.error("", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                                .getUserName(), "审计库管理", "测试备份审计库信息失败", 0);
		} finally {
			try {
				if (null != rs) {
					rs.close();
    ...
    //            String sqlPath = request.getRealPath("WEB-INF")+ "/init.sql";
                ...
                sqlExec.execute();
                logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                         .getUserName(), "恢复出厂设置", "初始化审计库信息成功", 1);

            } else {
                isResult = false;
                logger.error("恢复出厂设置--初始化数据库,权限不够");
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                      .getUserName(), "恢复出厂设置", "初始化审计库信息失败", 0);
            }
        } catch (Exception e) {
            isResult = false;
...
//                String dir = StringContext.systemPath + "/tomcat/webapps" + request.getContextPath() + "/data/waiwang/repository";
                ...
                deleteStatusService.deleteAll();
                logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                        .getUserName(), "恢复出厂设置", "初始化配置文件成功", 1);
            } else {
                isResult = false;
                logger.error("恢复出厂设置--初始化配置文件错误,权限不够");
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                      .getUserName(), "恢复出厂设置", "初始化配置文件失败", 0);
            }
        } catch (Exception e) {
            isResult = false;
    ...
    public String initSsh() throws Exception {
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result =	actionBase.actionBegin(request);
        ...
        try{
            if(StringUtils.isAuthUser(SessionUtils.getAccount(request))){
                ...
                if (osInfo.isLinux()) {
                    ...
                    String service = "service ssh start";
...
//                    result = "操作系统系统不对!";
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                        .getUserName(), "恢复出厂设置", "启动SSH成功", 1);
            } else {
                isResult = false;
                logger.error("恢复出厂设置--启动SSH错误,权限不够");
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                      .getUserName(), "恢复出厂设置", "初启动SSH失败", 0);
            }
        } catch (Exception e) {
            isResult = false;
            logger.error("恢复出厂设置--启动SSH错误,", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                      .getUserName(), "恢复出厂设置", "启动SSH失败", 0);
        }
    ...
    private void initConfig2(HttpServletRequest request) throws IOException {
        boolean privated = StringUtils.getPrivated();
        String targetDir = StringContext.systemPath+"/repository";
        File destDir = new File(targetDir);
        if(privated) {//内网
...
//                String dir = StringContext.systemPath + "/tomcat/webapps" + request.getContextPath() + "/data/waiwang/repository";
    ...
    public String importConfigTables() throws Exception {
        ...
        try{
            if(StringUtils.isAuthUser(SessionUtils.getAccount(request))){
                SAXReader reader = new SAXReader();
                String backFilePath = SiteContext.getInstance().contextRealPath
    ...
    //                    + AppConstant.XML_DB_CONFIG_PATH;
                ...
                String dbname = doc.selectSingleNode("//config/maindb/dbname").getText();
                String username = doc.selectSingleNode("//config/maindb/username").getText();
                Document ddoc = reader.read(new File(backFilePath));
                String conf_file_path = ddoc.selectSingleNode("//backup/conf_file_path").getText();
                String sqlPath = conf_file_path + "/" + file.split("\\(")[0];
//                SQLExec sqlExec = new SQLExec();
//                sqlExec.setEncoding("UTF-8");
//                //设置数据库参数
//                sqlExec.setDriver("com.mysql.jdbc.Driver");
...
//                sqlExec.execute();
                ...
                // 调用 mysql 的 cmd:
                Process child = rt.exec("mysql -u"+username+" -p"+password+" "+dbname+" ");
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                         .getUserName(), "审计库管理", "导入备份数据信息成功", 1);
                msg = "导入成功";
            } else {
                msg +=",未授权";
            }
            ...
            isResult = false;
            logger.error("审计库管理--用户导入备份数据,", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                      .getUserName(), "审计库管理", "导入备份数据信息失败", 0);
        }
...
//        }
	...
	public String importConfigTables_old() throws Exception {
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result =	actionBase.actionBegin(request);
	...
	public String showSqlFiles() throws Exception {
        ...
        try{
            ...
            if(files!=null&&files.length>0){
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                        .getUserName(), "审计库管理", "显示备份sql文件成功", 1);
            } else {
            ...
            logger.error("审计库管理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                  .getUserName(), "审计库管理", "显示备份sql文件失败", 0);
        }
        actionBase.actionEnd(response,json,result);
        return null;
	}
    ...
    public String selectBackup() throws Exception {
        ...
        try{
            ...
            json = "[{conf_type:'"+conf_type+"',conf_time:'"+conf_time+"',conf_day:'"+conf_day+
                    ...
                    "',log_time3:'"+log_time3+"'}]";
            logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                    .getUserName(), "审计备份策略", "显示审计备份策略成功", 1);
        } catch (Exception e){
            logger.error("审计备份策略",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                    .getUserName(), "审计备份策略", "显示审计备份策略失败", 0);
        }
        actionBase.actionEnd(response,json,result);
        return null;
    }
    ...
    public String updateBackup() throws Exception {
        ...
        try {
            ...
            msg = "保存成功";
            logService.newManagerLog("INFO", SessionUtils.getAccount(request)
                    .getUserName(), "审计备份策略", "更新审计备份策略信息成功", 1);
        } catch (Exception e) {
            logger.error("审计备份策略",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request)
                    .getUserName(), "审计备份策略", "更新审计备份策略信息失败", 0);
            msg = "保存失败";
        } finally {
            writer.close();
        }
