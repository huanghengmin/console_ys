...
//    private String remoteDisabled;
    ...
    public String select() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "安全策略","获取安全策略信息成功", 1);
        } catch (Exception e) {
            logger.error("安全策略", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "安全策略","获取安全策略信息失败", 0);
        }
    ...
    public String selectPasswordRules() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "管理","获取安全策略密码规则信息成功", 1);
        } catch (Exception e) {
            logger.error("管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "管理","获取安全策略密码规则信息失败", 0);
        }
        actionBase.actionEnd(response,json,result);
        return null;
    }
...
//            }
            ...
            logService.newManagerLog("INFO",  SessionUtils.getAccount(request).getUserName(), "安全策略","修改安全策略信息成功", 1);
        } catch (Exception e) {
            logger.error("安全策略", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "安全策略","修改安全策略信息失败", 0);
