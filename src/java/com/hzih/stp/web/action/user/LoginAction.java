...
public class LoginAction extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		Object obj = SiteContext.getInstance().loginErrorList.get(name);
		...
		if (obj != null && ((Integer) obj).intValue() >= errorLimit) {
			...
			securityService.addSecurityAlert("登陆服务器",new Date(),"0010","在"+ getIpAddr(request)+ "登陆超过"+errorLimit+"次口令错误","N",getIpAddr(request));
            logService.newSysLog("WARN","登陆","黑客入侵","在"+ getIpAddr(request)+ "登陆"+name+"超过"+errorLimit+"次口令错误");
			request.setAttribute("message", "超过" + errorLimit + "次口令错误,系统锁定,请联系系统管理员!");
			Account _account = loginService.getAccountByName(name);
            if(_account!=null){
                Set<Role> roles = _account.getRoles();
                RoleType roleType = null;
                for (Role r : roles){
                    roleType = roleService.getRoleType(r.getRoleType());
                }
                accountService.updateStatus(_account.getId(),"3");//锁定用户
                if(roleType.getAutoUnLock()==1){
                    new Timer().schedule(new TimerTask(){
                        public void run() {
                            SiteContext.getInstance().loginErrorList.remove(name);
                        }
		...
		if (account != null) {
            ...
            if(isTime){
                if(isIp){
                    ...
                    if(isUser(account)){
                        logService.newLog("INFO", account.getUserName(), "登录", "登录成功", 1);
                    } else {
                        logService.newManagerLog("INFO", account.getUserName(), "登录", "登录成功", 1);
                    }
                    // 登录错误数置零
                    Object _obj = SiteContext.getInstance().loginErrorList.get(name);
                    if (_obj != null) {
                        SiteContext.getInstance().loginErrorList.put(name, new Integer(0));
                    }
                    return "success";
                } else {
                    logger.info("非法登陆IP!");
                    ...
                    if(isUser(account)){
                        logService.newLog("INFO", name, "登录 ", " IP地址不允许访问 ", 0);
                    }else {
                        logService.newManagerLog("INFO", name, "登录 ", " IP地址不允许访问 ", 0);
                    }
                }
            } else {
                logger.info("非法登陆时间!");
                ...
                if(isUser(account)){
                    logService.newLog("INFO", name, "登录 ", " 该时间不允许访问 ", 0);
                } else {
                    logService.newManagerLog("INFO", name, "登录 ", " 该时间不允许访问 ", 0);
                }
            }
		} else {
            Account _account = loginService.getAccountByName(name);
            if(_account!=null){
                logger.info(" 密码错误！ ");
                if(isUser(_account)){
                    logService.newLog("INFO", name, "登录 ", " 密码错误 ", 0);
                } else {
                    logService.newManagerLog("INFO", name, "登录 ", " 密码错误 ", 0);
                }
                Object _obj = SiteContext.getInstance().loginErrorList.get(ip + name);
                if (_obj == null) {
                    SiteContext.getInstance().loginErrorList.put(ip + name, new Integer(1));
                } else {
                    SiteContext.getInstance().loginErrorList.put(ip + name, new Integer(((Integer) _obj).intValue() + 1));
                }
            } else {
                logger.info(" 用户名不存在！ ");
                logService.newSysLog("INFO", name, "登录", " 用户名不存在的登陆");
                Object _obj = SiteContext.getInstance().loginErrorList.get(ip);

                if (_obj == null) {
                    SiteContext.getInstance().loginErrorList.put(ip, new Integer(1));
                } else {
                    if (_obj != null && ((Integer) _obj).intValue() >= errorLimit) {
                        logger.info("同一IP"+ip+"超过" + errorLimit + "次使用空用户尝试登陆,请联系系统管理员! ");
                        securityService.addSecurityAlert("登陆服务器", new Date(), "0010", "在" + getIpAddr(request) + "登陆超过" + errorLimit + "次使用空用户", "N", getIpAddr(request));
                        logService.newSysLog("WARN","登陆","黑客入侵","在"+ getIpAddr(request)+ "登陆超过"+errorLimit+"次使用空用户");
                        request.setAttribute("message", "终端超过" + errorLimit	+ "次使用空用户,系统锁定,请联系系统管理员!");
                        new Timer().schedule(new TimerTask(){
                            public void run() {
