...
public class ProxyAction extends ActionSupport {
	...
	public String insert() throws Exception {
		...
		try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户新增通用代理信息成功", 1);
            msg = "新增成功,点击[确定]返回列表";
        } catch (Ex ex){
            logger.error("通用代理",ex);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户新增通用代理信息失败", 0);
            msg = "新增失败";
        }
        typeBase = null;
        typeData = null;
	...
	public String insertAuth() throws Exception {
        ...
        if(StringUtils.isAuthUserConfig(SessionUtils.getAccount(request))){
            try{
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户新增通用代理安全属性信息成功", 1);
                msg = "新增应用安全属性成功";
            } catch (Ex ex){
                logger.error("通用代理",ex);
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户新增通用代理安全属性信息失败", 0);
                msg = "新增应用安全属性失败";
            }
        }else {
            logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户新增通用代理安全属性信息权限不够", 0);
    ...
    public String update() throws Exception {
		...
		try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户更新通用代理信息成功", 1);
            msg = "更新成功,点击[确定]返回列表 ";
        } catch (Ex ex){
            logger.error("通用代理",ex);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户更新通用代理信息失败", 0);
            msg = "更新失败";
        }
        String json = "{success:true,msg:'"+msg+"'}";
        typeSafe = null;
    ...
    public String loadChannelPort() throws Exception {
		...
		try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户获取目标端通道端口信息成功", 1);
        } catch (Ex ex){
            logger.error("通用代理",ex);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户取目标端通道端口信息失败", 0);
        }
        String json = "{success:true,msg:'"+msg+"'}";
        base.actionEnd(response, json ,result);
		return null;
    ...
    public String updateAuth() throws Exception {
        ...
        if(StringUtils.isAuthUserConfig(SessionUtils.getAccount(request))){
            try{
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户修改通用代理安全属性信息成功", 1);
                msg = "修改安全属性成功";
            } catch (Ex ex){
                logger.error("通用代理",ex);
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户修改通用代理安全属性信息失败", 0);
                msg = "修改安全属性失败";
            }
        } else {
            logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户修改通用代理安全属性信息权限不够", 0);
    ...
    public String delete() throws IOException,Ex {
        ...
        try{
            ...
            logger.info("删除应用成功!");
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户删除通用代理信息提交成功", 1);
            msg = "等待授权用户授权";
        } catch (Exception e){
            logger.error("通用代理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户删除通用代理信息提交失败", 0);
            msg = "删除提交失败";
        }
        String json = "{success:true,msg:'"+msg+"'}";
		base.actionEnd(response, json ,result);
    ...
    public String readProxyQuery() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "端口映射", "用户查找可信和非可信通用代理信息成功", 1);
        } catch (Ex ex) {
            logger.error("端口映射",ex);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "端口映射", "用户查找可信和非可信通用代理信息失败", 0);
        }
        base.actionEnd(response, json ,result);
        return null;
    }
