...
public class DBTargetTableAction extends ActionSupport {
    ...
    public String readTargetTableNames() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取表集合列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取表集合列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String readTargetDBName() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取数据源下的所有对应源端的数据表名列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取数据源下的所有对应源端的数据表名列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String readTargetDBNameKeyValue() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取数据源"+dbName+"下的所有数据表名列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取数据源"+dbName+"下的所有数据表名列表失败", 0);
        }  finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String readTargetTableField() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取数据源"+dbName+"下的数据表"+tableName+"的列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取数据源"+dbName+"下的数据表"+tableName+"的列表失败", 0);
        }  finally {
            typeDB = null;
            typeBase = null;
        }
...
//                String targetDB = request.getParameter("targetDB");
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端"+_type+"时读取数据源的表的属性列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端"+_type+"时读取数据源的表的属性列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String updateSourceDBName() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端更新源端数据库和删除表集合成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端更新源端数据库和删除表集合失败", 0);
            msg = "更改失败,请重试或联系管理员";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String saveTargetTable() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步目标端表集合"+_operate+"表属性成功", 1);
            msg = _operate+"成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步目标端表集合" + _operate + "表属性失败", 0);
            msg = _operate+"失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String insertMoreTargetTableField() throws IOException {
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result = actionBase.actionBegin(request);
        ...
        try{
            for (int i = 0;i < tableNames.length; i ++ ) {
                Field[] tableFields = dataBaseService.readTableFields("internal", tableNames[i],typeTable.getTargetDBName(),sourceDBName,appName);
                typeTable.setDeleteEnable(deleteEnable);
                typeTable.setOnlyInsert(onlyInsert);
                typeTable.setSourceTableName(tableNames[i]);
                typeTable.setTableName(tableNames[i]);
                xmlOperatorService.updateTypeAllow(StringContext.INTERNAL, appName, false);
                xmlOperatorService.saveDBTypeTable(type,typeBase,typeDB,typeTable,tableFields);
            }

            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步目标端表集合批量新增表属性成功", 1);
            msg = "批量新增成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步目标端表集合批量新增表属性失败", 0);
            msg = "批量新增失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String deleteTargetTable() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步目标端表集合删除操作成功", 1);
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步目标端端表集合删除表失败", 0);
            msg = "删除失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String readTargetDBNamesInXml() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+srcTableName+"表下属性列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+srcTableName+"表下属性列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String deleteTargetDB() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步目标端表集合表"+srcTableName+"下的表删除操作成功", 1);
            msg = "删除成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步目标端端表集合表" + srcTableName + "下的表删除表失败", 0);
            msg = "删除失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String saveTargetDB() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步目标端表集合表"+srcTableName+"下的表新增操作成功", 1);
            msg = "保存成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步目标端端表集合表" + srcTableName + "下的表新增表失败", 0);
            msg = "保存失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String readTargetDBKeyValue() throws IOException {
        ...
        try{
            json = dataBaseService.readInternalDBKeyValue(dbName);
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+dbName+"数据源下表名列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+dbName+"数据源下表名列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String readTargetTableNamesInXml() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+srcTableName+"源表对应的目标数据库"+targetDB+"下的表名列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+srcTableName+"源表对应的目标数据库"+targetDB+"下的表名列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
    ...
    public String deleteTargetTableName() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端删除"+srcTableName+"源表对应的目标数据库"+targetDB+"下的表成功", 1);
            msg = "删除成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步应用" + appName + "目标端删除" + srcTableName + "源表对应的目标数据库" + targetDB + "下的表失败", 0);
            msg = "删除失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String saveTargetTableName() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端新增"+srcTableName+"源表对应的目标数据库"+targetDB+"下的表成功", 1);
            msg = "新增成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步应用" + appName + "目标端新增" + srcTableName + "源表对应的目标数据库" + targetDB + "下的表失败", 0);
            msg = "新增失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String readTargetTableAttribute() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+srcTableName+"源表对应的目标数据库"+targetDB+"下的表名为"+targetTable+"的属性成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"目标端读取修改时"+srcTableName+"源表对应的目标数据库"+targetDB+"下的表名为"+targetTable+"的属性失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
        }
