...
public class DBSourceTableAction extends ActionSupport {
    ...
    public String readSourceTableNames() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"源端读取数据源"+dbName+"下表集合列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"源端读取数据源"+dbName+"下表集合列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
            typeTable = null;
    ...
    public String readSourcedDBName() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"源端读取数据源"+dbName+"下的所有表名列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"源端读取数据源"+dbName+"下的所有表名列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
            typeTable = null;
    ...
    public String readSourceTableField() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"源端"+_type+"时读取数据源"+dbName+"下的表"+tableName+"的属性列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步应用"+appName+"源端"+_type+"时读取数据源"+dbName+"下的表"+tableName+"的属性列表失败", 0);
        } finally {
            typeDB = null;
            typeBase = null;
            typeTable = null;
    ...
    public String saveSourceTable() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步源端表集合"+_operate+"表属性成功", 1);
            msg = _operate+"成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步源端表集合"+_operate+"表属性失败", 0);
            msg = _operate+"失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String insertMoreSourceTableField() throws IOException {
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result = actionBase.actionBegin(request);
        ...
        try{
            for (int i = 0;i < tableNames.length; i ++ ) {
                Field[] tableFields = dataBaseService.readTableFields("external", tableNames[i],dbName);
                typeTable.setDelete(monitordelete);
                typeTable.setUpdate(monitorupdate);
                typeTable.setInsert(monitorinsert);
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步源端表集合批量新增表属性成功", 1);
            msg = "批量"+_operate+"成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步源端表集合批量新增表属性失败", 0);
            msg = "批量"+_operate+"失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String deleteSourceTable() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步源端表集合删除操作成功", 1);
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步源端表集合删除表失败", 0);
            msg = "删除失败";
        } finally {
            typeDB = null;
            typeBase = null;
    ...
    public String deleteBackUpSourceTable() throws IOException {
        ...
        try{
            ...
            xmlOperatorService.updateTypeAppSend(appName,StringContext.UPDATE_APP);
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步源端表集合删除操作恢复成功", 1);
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据库同步", "数据库同步源端表集合删除表恢复失败", 0);
            msg = "删除恢复失败";
        } finally {
            typeDB = null;
            typeBase = null;
