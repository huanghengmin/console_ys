...
public class ProxyIpMacAction extends ActionSupport{
	...
	public String readIpMac() throws Exception {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理","读取"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单成功!", 1);
        } catch (Exception e){
            logger.error("通用代理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理","读取"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单失败!", 0);
        }
	...
	public String saveIpMac() throws Exception {
        ...
        String result =	base.actionBegin(request);
        String msg = null;
        if(StringUtils.isAuthUserConfig(SessionUtils.getAccount(request))){
            try{
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理","保存"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单成功!", 1);
                msg = "保存成功,点击[确定]返回列表!";
            } catch (Exception e){
                logger.error("通用代理", e);
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理","保存"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单失败!", 0);
                msg = "保存失败";
            }
        }  else {
            logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户新增通用代理安全属性信息IP/MAC黑白名单权限不够", 0);
	...
	public String deleteIpMac() throws Exception {
        ...
        if(StringUtils.isAuthUserConfig(SessionUtils.getAccount(request))){
            try{
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理","删除"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单成功!", 1);
                msg = "删除成功,点击[确定]返回列表!";
            } catch (Exception e){
                logger.error("通用代理",e);
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理","删除"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单失败!", 0);
                msg = "删除失败";
            }
        }  else {
            logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户删除通用代理安全属性信息IP/MAC黑白名单权限不够", 0);
	...
	public String updateIpMac() throws Exception {
        ...
        if(StringUtils.isAuthUserConfig(SessionUtils.getAccount(request))){
            try{
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理","修改"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单成功!", 1);
                msg = "修改成功,点击[确定]返回列表!";
            } catch (Exception e){
                logger.error("通用代理",e);
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理","修改"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单失败!", 0);
                msg = "修改失败";
            }
        }  else {
            logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户修改通用代理安全属性信息IP/MAC黑白名单权限不够", 0);
    ...
    public String checkIp() throws Exception {
        ...
        if(StringUtils.isAuthUserConfig(SessionUtils.getAccount(request))){
            try{
                ...
                logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "通用代理","校验"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单"+ip+"是否存在成功!", 1);
            } catch (Exception e){
                logger.error("通用代理", e);
                logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "通用代理","校验"+(typeXml.equals("internal")?"可信":"非可信")+"配置文件的"+proxyType+"代理应用"+appName+"的IP/MAC"+(ipMacType.equals("black")?"黑":"白")+"名单"+ip+"是否存在失败!", 0);
                msg = "校验失败,可能无法继续操作";
            }
        }  else {
            logService.newManagerLog("WARN", SessionUtils.getAccount(request).getUserName(), "通用代理", "用户校验通用代理安全属性信息IP/MAC黑白名单权限不够", 0);
