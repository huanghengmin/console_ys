...
public class JdbcAction extends ActionSupport {
    ...
    public String insert() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据源管理","新增JDBC成功", 1);
            msg = "新增成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据源管理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据源管理","新增JDBC失败", 0);
            msg = "新增失败";
        } finally {
            typeBase = null;
            jdbc = null;
    ...
    public String delete() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据源管理",msg, 1);
        } catch (Exception e){
            logger.error("数据源管理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据源管理","数据库同步新增失败", 0);
            msg = "删除失败";
        }finally {
            typeBase = null;
            jdbc = null;
...
//            }
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据源管理","修改JDBC成功", 1);
            msg = "修改成功,点击[确定]返回列表";
        } catch (Exception e){
            logger.error("数据源管理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据源管理","修改JDBC失败", 0);
            msg = "修改失败";
        } finally {
            typeBase = null;
            jdbc = null;
    ...
    public String checkUsing() throws IOException{
        HttpServletRequest request = ServletActionContext.getRequest();
        HttpServletResponse response = ServletActionContext.getResponse();
        ActionBase actionBase = new ActionBase();
        String result = actionBase.actionBegin(request);
        ...
        String appUsing = "";
        try{
            appUsing = xmlOperatorService.checkJdbcNameIsNowUsed (typeBase.getPrivated(),jdbc.getJdbcName());
        }  catch (Exception e){
            logger.error("数据源管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据源管理","检查数据源使用情况失败", 0);
        } finally {
            typeBase = null;
            jdbc = null;
        }
        json = "{success:true,msg:'" + appUsing + "'}";
        actionBase.actionEnd(response,json,result);
        return null;
    ...
    public String select() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据源管理","数据库同步新增成功", 1);
        } catch (Exception e){
            logger.error("数据源管理", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据源管理","数据库同步新增失败", 0);
        } finally {
    ...
    public String testConnect() throws IOException {
        ...
        String msg = null;
        try{
            String test = request.getParameter("jdbc.dbUrl");
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据源管理","JDBC测试成功", 1);
        } catch (Exception e){
            logger.error("数据源管理",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据源管理","JDBC测试失败", 0);
            msg = "测试失败";
        }finally {
            typeBase = null;
            jdbc = null;
    ...
    public String readJdbcName() throws IOException {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步新增或修改时获取数据源名列表成功", 1);
        } catch (Exception e){
            logger.error("数据库同步",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"数据库同步","数据库同步新增或修改时获取数据源名列表失败", 0);
        } finally {
            typeBase = null;
            jdbc = null;
        }
    ...
    public String check() throws Exception {
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"新增数据源","新增数据源"+jdbcName+"验证成功!", 1);
            logger.info("新增数据源验证成功!");
        } catch (Ex e){
            logger.error("新增数据源验证",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"新增数据源","新增数据源"+jdbcName+"验证失败!", 0);
            msg = "验证失败,请联系管理员!";
        }finally {
            typeBase = null;
            jdbc = null;
    ...
    public String readDataBaseNamesKeyValue() throws Exception {
        ...
        try{
            ...
            json += "]}";
            logService.newManagerLog("INFO",SessionUtils.getAccount(request).getUserName(),"数据源","读取数据库类型成功", 1);
        } catch (Exception e) {
            logger.error("读取数据库类型错误",e);
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"数据源","读取数据库类型失败", 0);
    ...
    public String readDataBaseVersionsKeyValue() throws Exception {
        ...
        try{
            ...
            json += "]}";
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"数据源","读取数据库类型"+dbType+"对应的版本成功", 1);
        } catch (Exception e) {
            logger.error("读取数据库类型"+dbType+"对应的版本错误",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据源", "读取数据库类型" + dbType + "对应的版本失败", 0);
            json = "{success:true,total:0,rows:[]}";
        } finally {
            typeBase = null;
            jdbc = null;
    ...
    public String readDataBaseDriversKeyValue() throws Exception {
        ...
        try{
            ...
            json += "]}";
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"数据源","读取数据库类型"+dbType+"对应版本"+version+"的驱动成功", 1);
        } catch (Exception e) {
            logger.error("读取数据库类型"+dbType+"对应版本"+version+"的驱动错误",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "数据源", "读取数据库类型" + dbType + "对应版本" + version + "的驱动失败", 0);
            json = "{success:true,total:0,rows:[]}";
        } finally {
            typeBase = null;
            jdbc = null;
    ...
    public String readDriverInfo() throws Exception {
        ...
        try{
            ...
            logService.newManagerLog("INFO",SessionUtils.getAccount(request).getUserName(),"数据源","读取数据库类型"+dbType+"对应版本"+version+"的驱动"+driver+"的对应信息成功", 1);
        } catch (Exception e) {
            logger.error("读取数据库类型错误",e);
            logService.newManagerLog("ERROR",SessionUtils.getAccount(request).getUserName(),"数据源","读取数据库类型"+dbType+"对应版本"+version+"的驱动"+driver+"的对应信息失败", 0);
