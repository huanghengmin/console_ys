...
public class AuditResetAction extends ActionSupport {
    ...
    public String select() throws IOException {
        ...
        try {
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "业务手动重传","读取业务手动重传信息成功 ", 1);
        } catch (Exception e) {
            logger.error("业务手动重传", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "业务手动重传","读取业务手动重传信息失败 ", 0);
        }
    ...
    public String insert() throws Exception {
        ...
        try {
            ...
            msg = "导入成功,点击[确定]返回列表!";
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "业务手动重传","导入业务手动重传信息成功 ", 1);
        } catch (Exception e) {
            logger.error("业务手动重传", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "业务手动重传","导入业务手动重传信息失败 ", 0);
    ...
    public String update() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "业务手动重传","重传业务手动重传信息成功 ", 1);
        } catch (Exception e) {
            logger.error("业务手动重传", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "业务手动重传","重传业务手动重传信息失败 ", 0);
    ...
    public String updateByBusinessName() throws Exception {
        ...
        try {
            ...
            msg = auditResetService.updateResetStatusByBizName(appName,startDate,endDate);//按业务名,时间重传
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "业务手动重传","重传业务"+appName+"成功 ", 1);
        } catch (Exception e) {
            logger.error("业务手动重传", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "业务手动重传","重传业务"+appName+"失败 ", 0);
            msg = "重传失败"+e.getMessage();
        }
        String json =  "{success:true,msg:'"+msg+"'}";
        actionBase.actionEnd(response, json, result);
    ...
    public String truncate() throws Exception {
        ...
        try {
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(), "业务手动重传","清空业务手动重传信息成功 ", 1);
            msg += "成功,点击[确定]返回列表!";
        } catch (Exception e) {
            logger.error("业务手动重传", e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(), "业务手动重传","清空业务手动重传信息失败 ", 0);
            msg += "失败"+e.getMessage();
        }
        String json =  "{success:true,msg:'"+msg+"'}";
        actionBase.actionEnd(response, json, result);
