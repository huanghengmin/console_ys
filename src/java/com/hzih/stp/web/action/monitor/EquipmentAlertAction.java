...
public class EquipmentAlertAction extends ActionSupport {
    ...
    public String select() throws IOException{
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"设备运行监控","读取设备CPU,内存和硬盘的告警阀值成功!", 1);
        } catch (Exception e){
            logger.error("设备运行监控",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"设备运行监控","读取设备CPU,内存和硬盘的告警阀值失败!", 0);
    ...
    public String insertOrUpdate() throws IOException{
        ...
        try{
            ...
            logService.newManagerLog("INFO", SessionUtils.getAccount(request).getUserName(),"设备运行监控","设置设备CPU,内存和硬盘的告警阀值成功!", 1);

        } catch (Exception e){
            logger.error("设备运行监控",e);
            logService.newManagerLog("ERROR", SessionUtils.getAccount(request).getUserName(),"设备运行监控","设置设备CPU,内存和硬盘的告警阀值失败!", 0);
            msg = "设置失败";
        }
