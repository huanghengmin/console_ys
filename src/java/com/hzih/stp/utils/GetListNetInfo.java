...
public class GetListNetInfo{
	...
	/**
	 * 读取/etc/network/interfaces文件中的所有启动时启用的网络网口
	 * @param lines 所有行
	 * @return
	 */
    ...
    /**
     * 读取/etc/network/interfaces文件中的所有网络网口
     * @param lines
     * @return
     */
    ...
    /**
     * 调用"+StringContext.systemPath+"/console.sh/config_update.sh修改网络网口信息
     *
     * @param netInfo
     * @param isUp
     * @param gatewayOlder
...
//	}
	...
	public String deleteInterface(String interfaceName ) throws Exception{
        ...
        if(osInfo.isLinux()){
            ...
            proc.exec(command2);
            // logger.info(command);
            return isDeleteInterface?"网络网口关闭成功,点击返回列表!":"网络网口关闭失败!";
        }else if(osInfo.isWin()){
...
//            logger.info(result);
            if(result.indexOf("Error") > -1 ){
                ...
                String command2 = "arp -f";
                proc.exec(command2);
                return "网络网口激活成功,点击返回列表!";
            }
//            return isDeleteInterface?"网络网口激活成功,点击返回列表!":"网络激活失败!";
        }else if(osInfo.isWin()){
	...
	public String ifDown(String interfaceName) throws Exception{
        ...
        if(osInfo.isLinux()){
            ...
            proc.exec(command2);
            // logger.info(command);
            return isDeleteInterface?"网络网口注销成功,点击返回列表!":"网络网口注销失败!";
        }else if(osInfo.isWin()){
	...
	public String saveRouter(NetInfo netInfo) throws Exception{
		...
		if(osInfo.isLinux()){
            ...
            if(netInfo.getGateway() != null && netInfo.getGateway().length() > 0 && netInfo.getGateway().indexOf("请输入IP(例:1.1.1.1)") <= -1){
                command = command +" gw "+netInfo.getGateway();
            }
            command = command +" -n "+netInfo.getInterfaceName();
           	proc.exec(command);
