        ...
        try{
            ...
            if(connector != null){
                ...
                if(result.equals("保存成功")){
                    if(port.equals(""+8443)){
                        return "更新管理服务网口设定IP地址成功";
                    }else if(port.equals(""+8000)){
                        return "更新集控采集数据网口设定IP地址成功";
                    }else{
...
/**********************************************************************************************************************/
    public void addType(Type type, String appType) throws Ex {
        ...
        child = typenode.addElement("securityFlag");
        child.setText(type.getSecurityFlag()==null?"":type.getSecurityFlag());
    ...
    public void addSourceFile(Element sourcefile, SourceFile sourceFile) {
    	if(sourceFile!=null){
            ...
            //add by ztt backup info
            child = sourcefile.addElement("isBackUp");
            child.setText(""+sourceFile.isBackUp());
            child = sourcefile.addElement("backUpDir");
            child.setText(sourceFile.getBackUpDir());
...
//        }
    ...
    public void addSourceField(String appName, String dbName, String tableName, Field field) throws Ex {
        ...
        if (fields != null) {
            ...
            if(jdbc != null && jdbc.getDbType().toLowerCase().equals("oracle") && field.getJdbcType().toLowerCase().equals("date")){
                child.setText("timestamp");
            }else{
...
//        }
    ...
    public void editType(Type type, String appType) throws Ex {
        ...
        if (typeNode != null) {
            ...
            typeNode.element("securityFlag").setText(type.getSecurityFlag()==null?"":type.getSecurityFlag());
...
//        		Element sourcefile = change.element("sourcefile");
                ...
                //add by ztt backup info
                sourcefile.element("isBackUp").setText("" + sourceFile.isBackUp());
                sourcefile.element("backUpDir").setText(sourceFile.getBackUpDir());
...
//            throw new Ex().set(E.E_NullPointer, E.KEY_NULLPOINTER, "jdbc�ڵ�");
    ...
    /**
     * 获取所有正在启用且使用数据源dbName的应用
     * @param dbName
     * @return
     */
    public String getAppUsingByDBName (String dbName){
        String appName = "";
        List typeNodes = document.selectNodes("/configuration/system/ichange/types/type[@apptype='db']");
        if(typeNodes != null){
            for (Iterator it = typeNodes.iterator(); it.hasNext();) {
                Element tp = (Element) it.next();
                if( "true".equalsIgnoreCase(tp.element("isactive").getText())){
                    Element database = (Element) tp.selectSingleNode("plugin/sourceplugin/dbchange/database[@name='"+dbName+"']");
                    if(database != null){
                        appName += tp.attribute("value").getText() + ",";
                    }
                }
            }
        }
...
//		List tableNodes = document.selectNodes("/configuration/system/ichange/types/type/plugin/targetplugin/dbchange/srcdb/tables/srctable/targetdb[@value='"+dbName+"']");
    ...
    public String getAppUsingByDBNameTarget(String dbName){
        String appName = "";
        List typeNodes = document.selectNodes("/configuration/system/ichange/types/type[@apptype='db']");
        if(typeNodes != null){
            for (Iterator it = typeNodes.iterator(); it.hasNext();) {
                Element tp = (Element) it.next();
                if( "true".equalsIgnoreCase(tp.element("isactive").getText())){
                    Element database = (Element) tp.selectSingleNode("plugin/targetplugin/dbchange/srcdb/tables/srctable/targetdb[@value='\"+dbName+\"']");
                    if(database != null){
                        appName += tp.attribute("value").getText() + ";";
                    }
                }
...
//            throw new Ex().set(E.E_NullPointer, E.KEY_NULLPOINTER, "database 为空");
    ...
    public SourceFile getSourceFile(String appName, String pluginType) throws Ex {
        ...
        if (file != null) {
            ...
            //add by ztt backup info
            sourceFile.setBackUp(file.element("isBackUp") != null ? file.element("isBackUp").getText() :"false");
            sourceFile.setBackUpDir(file.element("isBackUp") != null ?file.element("backUpDir").getText() : "");
...
//            output = new XMLWriter(new FileWriter(file), format);
    ...
    public void editIChangeUtils(ChannelIChangeUtils cic) {
        ...
        if(iChangeUtils!=null){
            ...
            if(iChangeUtils.element("backupdirdelete")!=null){
                iChangeUtils.element("backupdirdelete").setText(String.valueOf(cic.getBackupdirdelete()));
            }else{
                child = iChangeUtils.addElement("backupdirdelete");
                child.setText(String.valueOf(cic.getBackupdirdelete()));
            }
    ...
    public void editBackUpBase(BackUp backUp) throws Ex {
        Element base = (Element)document.selectSingleNode("/backup/base");
        if(base!=null){
    ...
    public Map<String,String>  getAlertEmail() {
