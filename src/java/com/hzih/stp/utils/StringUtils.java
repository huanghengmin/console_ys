...
//        if("002".equals(role.getRoleType())){
    ...
    public static boolean isAuthUserConfig(Account account){
        Set<Role> roles =  account.getRoles();
        Iterator<Role> iterator = roles.iterator();
        Role role = null;
        while (iterator.hasNext()){
...
//        if("002".equals(role.getRoleType())||"006".equals(role.getRoleType())){
        if(role.getRoleType() == 2){
            return true;
        }
        return false;
    ...
    public String deleteString(String[] ips, String ip) { //去掉ips中等于ip的数�?
        String ip_str = "";
        for (int i = 0; i < ips.length; i++) {
            if(!ip.equals(ips[i])){
                ip_str += ","+ips[i];
    ...
    /**
     * get temp data path for audit reset
     * @return
     */
    public static String getTempDataDir(){
        ...
        String dataTempDir = "";
        if (osInfo.isWin()) {
            dataTempDir = System.getProperty("ichange.home") + "/data/temp";
    ...
    public static String getEtc(){
        ...
        String etcDir = "";
        if(osInfo.isWin()){
            etcDir = System.getProperty("ichange.home") + "/etc";
        }else if(osInfo.isLinux()){
            etcDir = "/etc";
        }
        return etcDir;
