...
//	private Logger log = Logger.getLogger(XmlOperatorDAOImpl.class);
    ...
    public void updateSecurityFile(TypeBase typeBase) throws Ex {
        ...
        type.setVirusScan(String.valueOf(typeBase.isVirusScan()));
        type.setFilter(String.valueOf(typeBase.isFilter()));
        type.setSecurityFlag(typeBase.getSecurityFlag());
        config.editType(type,type.getAppType());
        getConfigPrivatedEnd(config,typeBase.getPrivated());
    }
	...
	public void saveFileType(TypeBase typeBase, TypeFile typeFile) throws Ex {
		...
		if("1".equals(typeBase.getPlugin())){
            ...
            //add by ztt file backup info
            sourceFile.setBackUp(String.valueOf(typeFile.getBackUp()));
            sourceFile.setBackUpDir(typeFile.getBackUpDir() == null || "请输入不同于源文件目录的目录".equals(typeFile.getBackUpDir()) ?"":typeFile.getBackUpDir());
	...
	public void updateFileType(TypeBase typeBase, TypeFile typeFile) throws Ex {
		...
		if(typeFile == null){
			...
			if("1".equals(typeBase.getPlugin())){
                ...
                //add by ztt file backup info
                sourceFile.setBackUp(String.valueOf(typeFile.getBackUp()));
                sourceFile.setBackUpDir("请输入不同于源文件目录的目录".equals(typeFile.getBackUpDir()) || typeFile.getBackUpDir() == null?"":typeFile.getBackUpDir());
	...
	public void saveDBType(TypeBase typeBase, TypeDB typeDB) throws Ex {
        ...
        type.setChannel(typeBase.getChannel());
        type.setChannelPort(typeBase.getChannelport());
        type.setDataPath(typeBase.getDataPath()==null?"/usr/app/stp/dest":typeBase.getDataPath());
...
//    				iDataBase.createSequence();
    ...
    private boolean needOperateDB(Table table, TypeTable typeTable, String dbName, Configuration config, String appName, Field[] tableFields) throws Ex {
        ...
        for(int i=0;i<tableFields.length;i++){
            ...
            if(field != null && tableFields[i] != null && field.equals(tableFields[i])){
                continue;
            } else {
	...
	public List<String> deleteJdbcByName(String[] jdbcNameArray, String typeXml) throws Ex {
		...
		for (int i = 0; i < jdbcNameArray.length; i++) {
			...
			if(!isUsed){
                ...
                try{
                    Jdbc jdbc = getExternalJdbcByName(jdbcNameArray[i]);
                    String dblocal = Constant.DB_INTERNAL;
                    iDataBase = DBFactory.getDataBase(jdbc, dblocal);
                    if(iDataBase.isConnectable()){
                        iDataBase.openConnection();
                        if("ORACLE".equals(jdbc.getDbVender())){					//oracle function
                            //todo
                            iDataBase.deleteFunction();
                        }
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }finally {
                    if(iDataBase != null && iDataBase.isConnectable()){
                        iDataBase.closeConnection();
                    }
                }
			}else{
	...
	public void saveJdbc(Jdbc jdbc, Boolean privated) throws Ex {
        ...
        //add by ztt:for more temp tables
        String dblocal = null;
        if(privated){
            dblocal = Constant.DB_INTERNAL;
        }else{
...
//		config.addTargetTable(srcTable, appName);
    ...
    @Override
    public String getAppsByJdbcName(boolean privated, String jdbcName) throws Ex {
        Configuration config = getConfigPrivatedHead(privated);
        if(privated){
            return config.getAppUsingByDBNameTarget(jdbcName);
        } else {
            return config.getAppUsingByDBName(jdbcName);
        }
    }
    ...
    @Override
    public Field getSourceFiled(String typeXml, String appName, String dbName, String tableName, String fieldName) throws Ex {
        Configuration config = getConfigTypeXmlHead(typeXml);
    ...
    public SourceFile getSourceFile(String appName) throws Ex {
        Configuration config = getConfigPrivatedHead(false);
        return config.getSourceFile(appName,Plugin.s_source_plugin);
    }

    public TargetFile getTargetFile(String appName) throws Ex {
        Configuration config = getConfigPrivatedHead(true);
        return config.getTargetFile(appName, Plugin.s_target_plugin);
    }
	...
	public void updateDBTypeSourceData(TypeBase typeBase,TypeDB typeDB) throws Ex {
		...
		if(!typeDB.getDbName().equals(config.getSourceDbName(appName))){
			...
			if("".equals(_dataBase.getTempTable())){
				...
				if(isRemove){
					deleteTypeTableTriggerOrFlag(operation, jdbc, dblocal, triggers, tempTable, tableNames,appName);
				}
			}
...
//        }
    ...
    @Override
    public String[] getSNMPClient(boolean isPrivate) throws Ex {
    ...
    @Override
    public String[] getSysLogClient(boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
    ...
    @Override
    public void saveSNMPClient(String ip, boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
    ...
    @Override
    public void saveSysLogClient(String ip, boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
    ...
    @Override
    public void deleteSNMPClient(String ip, boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
    ...
    @Override
    public void deleteSysLogClient(String ip, boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
    ...
    @Override
    public void updateSNMPClient(String snmpclient, String oldSNMPClient, boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
    ...
    @Override
    public void updateSysLogClient(String syslogclient, String oldSysLogClient, boolean isPrivate) throws Ex {
        Configuration config;
        if(isPrivate){
            config = new Configuration(StringContext.INTERNALXML);
        } else {
...
//        	iDataBase.removeTempTable(tempTable);			//delete tempTable
	...
	public void deleteTypeTableTriggerOrFlag(String operation,Jdbc jdbc,String dblocal,TriggerBean[] triggers,String tempTable,String[] tableNames , String appName) throws Ex {
		if("trigger".equals(operation)){
        	IDataBase iDataBase = DBFactory.getDataBase(jdbc, dblocal);
            try{
                iDataBase.openConnection();
