...
public class DeleteStatusDaoImpl extends MyDaoSupport implements DeleteStatusDao{
    ...
    public List<DeleteStatus> findByFlagType(int flagType) throws Exception {
        String hql = new String("from DeleteStatus where flagType <= ?");
        List<DeleteStatus> list = getHibernateTemplate().find(hql,new Integer[] { flagType });
        return list;
    }

    @Override
    public void deleteAll() throws Exception {
        String sql = "delete from delete_status where id > 0" ;
        Session session = getSession();
        SQLQuery sqlQuery = session.createSQLQuery(sql);
        sqlQuery.executeUpdate();
    }
