...
public class DataBaseServiceImpl implements DataBaseService {
	...
	public String readInternalDB(Integer start, Integer limit, String dbName) throws Ex {
		...
		if(total == 0){
			...
			int index = 0;
			for (int i = 0; i < total; i++) {
				if(i==start&&index<limit && tableNames[i].indexOf("'")<0 ){
					start ++;
					index ++;
					json +="{'tableName':'"+tableNames[i]+"','index':'1'},";
				}
...
//                }
    ...
    public class FieldComparator implements Comparator {
        public int compare(Object object, Object object1) {
            ...
            if(field1.isPk() == true && field2.isPk() == false){
                return -1;
            }
            if (field1.isPk() == false && field2.isPk() == true){
                return  1;
            }
            if(field1.getJdbcType().toLowerCase().equals("date") || field1.getJdbcType().toLowerCase().equals("timestamp") || field1.getDbType().toLowerCase().equals("date") || field1.getDbType().toLowerCase().equals("timestamp")){
                return -1;
            }
            if(field2.getJdbcType().toLowerCase().equals("date") || field2.getJdbcType().toLowerCase().equals("timestamp") ||field2.getDbType().toLowerCase().equals("date") || field2.getDbType().toLowerCase().equals("timestamp") ){
                return 1;
            }
            return 0;
        }
	...
	public String readTableFieldExist(Integer start, Integer limit, String typeXml, String appName, String tableName, String dbName) throws Ex {
        ...
        FieldComparator comparator = new FieldComparator();
        Collections.sort(fieldsInDB, comparator);
		db.closeConnection();
...
//                if(index == start && count < limit){
                    Field f = xmlOperatorDAO.getSourceFiled(typeXml,appName,dbName,tableName,field.getFieldName());
                    if(f!=null){
...
//                index ++;
    ...
    @Override
    public Field[] readTableFields(String typeXml, String tableName, String dbName) throws Ex {
        Jdbc jdbc = null;
        IDataBase db = null;
        if("external".equals(typeXml)){
            jdbc = xmlOperatorDAO.getExternalJdbcByName(dbName);
        ...
        List<Field> fieldsInDB = db.getFields(tableName);
        db.closeConnection();
        Field[] fields = new Field[fieldsInDB.size()];
        for( int i = 0; i < fieldsInDB.size(); i++ ) {
            Field field = fieldsInDB.get(i);
            if("internal".equals(typeXml)) {
                field.setDestField(field.getFieldName());
            }
            fields[i] = field;
        }
        return fields;
    ...
    @Override
    public Field[] readTableFields(String typeXml, String tableName, String t_dbName , String s_dbName , String appName) throws Ex {
        Jdbc jdbc = null;
		IDataBase db = null;
		if("external".equals(typeXml)){
			jdbc = xmlOperatorDAO.getExternalJdbcByName(t_dbName);
			db = DBFactory.getDataBase(jdbc, Constant.DB_INTERNAL);
		}else if("internal".equals(typeXml)){
			jdbc = xmlOperatorDAO.getInternalJdbcByName(t_dbName);
			db = DBFactory.getDataBase(jdbc, Constant.DB_INTERNAL);

		}
		db.openConnection();
		List<Field> fieldsInDB = db.getFields(tableName);
        List<Field> sourcefields = xmlOperatorDAO.getExternalTypeFields(appName, s_dbName, tableName);
        String pk_FieldName = null;
        for(int i = 0 ;i < sourcefields.size() ; i++){
            if(sourcefields.get(i).isPk()){
                pk_FieldName = sourcefields.get(i).getFieldName();
                break;
            }
        }
        db.closeConnection();
        ...
        for( int i = 0; i < fieldsInDB.size(); i++ ) {
            ...
            if("internal".equals(typeXml)) {
                field.setDestField(field.getFieldName());
                if(pk_FieldName != null && field.getFieldName().equalsIgnoreCase(pk_FieldName)){
                    field.setPk("true");
                }
            }
            fields[i] = field;
...
//		String fieldValue = ""+field.getFieldName()+";"+jdbcType+";"+field.getColumnSize()+";"+field.isNull()+";"+field.getDbType();
	...
	public String checkTempTable(String typeXml, String tempTable, String dbName) throws Ex {
		Jdbc jdbc = null;
		String dblocal = null;
        Map<String,String> db_tableName = new HashMap<>();
		if("external".equals(typeXml)){
			jdbc = xmlOperatorDAO.getExternalJdbcByName(dbName);
		...
		String msg = null;
        for(int i = 0 ; i< total ; i++){
            if(!db_tableName.containsKey(tableNames[i])){
                db_tableName.put(tableNames[i].toUpperCase(),tableNames[i].toUpperCase());
            }
        }
        String [] tempTables = tempTable.trim().split(",");
		for (int i = 0; i < tempTables.length; i++) {
            if(db_tableName.containsKey(tempTables[i].toUpperCase())){
                msg = "表 " + tempTables[i] + " 已存在!";
                return msg;
            }
		}
		msg = "0000";
    ...
    @Override
    public String checkUpdateTempTable(String typeXml, String tempTable, String oldTempTable, String dbName) throws Ex {
        Jdbc jdbc = null;
        String dblocal = null;
        Map<String,String> db_tableName = new HashMap<>();
        Map<String,String> old_tempTable = new HashMap<>();
        if("external".equals(typeXml)){
            jdbc = xmlOperatorDAO.getExternalJdbcByName(dbName);
            dblocal = Constant.DB_INTERNAL;
        }else if("internal".equals(typeXml)){
        ...
        String [] tempTables = tempTable.trim().split(",");
        String [] oldTempTables = oldTempTable.trim().split(",");
        for(int i = 0 ;i < oldTempTables.length ; i++){
            if(!old_tempTable.containsKey(oldTempTables[i])){
                old_tempTable.put(oldTempTables[i].toUpperCase(),oldTempTables[i].toUpperCase());
            }
        }
        for (int i = 0; i < tempTables.length; i++) {
            if(old_tempTable.containsKey(tempTables[i].toUpperCase())){
                continue;
            }
            if(db_tableName.containsKey(tempTables[i].toUpperCase())){
                msg = "表 " + tempTables[i] + " 已存在!";
    ...
    public void operateDBRemoveApp(String typeXml, String appName) throws Ex {
        ...
        for (DataBase dataBase : list) {
            ...
            if(StringUtils.isNotBlank(dataBase.getTempTable())){
                removeTempTable(typeXml, dataBase,appName);
            }
        }
    }
    ...
    private void removeDataBase(String typeXml, String appName, DataBase dataBase) throws Ex {
        ...
        removeSequence(typeXml, appName, dataBase);
    }
    ...
    private void editTables(String typeXml, String appName, DataBase dataBase) throws Ex {
        ...
        for(Table table : list){
            if(table.getStatus().equals(StringContext.DB_DELETE)){
                ...
                }else if(dataBase.getOperation().equals("addtrigger")){
                    addMoreTigger(typeXml,appName,dataBase,table);
                    if(StringUtils.isNotBlank(table.getFlag())){
                        removeFlag(typeXml,appName,dataBase.getDbName(),table);//删除数据库中的标记
                        table.setFlag("");
                    }
     ...
     * @param dataBase
     */
    private void removeTempTable(String typeXml, DataBase dataBase,String appName) throws Ex {
        boolean result = false;
        IDataBase iDataBase = null;
        try{
            Jdbc jdbc = xmlOperatorDAO.getJdbcByName(typeXml, dataBase.getDbName());
            iDataBase = DBFactory.getDataBase(jdbc, Constant.DB_INTERNAL);
            iDataBase.openConnection();
            ...
            if(isExistTempTable){
                if("ORACLE".equals(jdbc.getDbVender())){		//oracle sequence
//                iDataBase.removeSequence();
                    iDataBase.removeMoreSequence(appName);
                }
    ...
    private boolean isExistTempTable(String tempTable, String[] tableNames) {
        for (int i = 0; i < tableNames.length; i++) {
			if(tempTable.toUpperCase().equals(tableNames[i].toUpperCase())){
                return true;
			}
		}
        return false;
    ...
    /**
     * 多个临时表的情况
     * 加触发器,返回触发器几何,修改表中触发器名称
     * @throws Ex
     */
    private void addMoreTigger(String typeXml, String appName, DataBase dataBase, Table table) throws Ex{
        Map<String,String> triggers =addMoreTriggerInDB(typeXml,appName,dataBase,table);   //直接连接数据库添加触发器
        table.setStatus("");      //去掉状态
        String deleteTrigger = triggers.get("delete");
        String insertTrigger = triggers.get("insert");
        String updateTrigger = triggers.get("update");
     ...
     * @throws Ex
     */
    private Map<String, String> addMoreTriggerInDB(String typeXml, String appName, DataBase dataBase, Table table) throws Ex {
        Jdbc jdbc = xmlOperatorDAO.getJdbcByName(typeXml, dataBase.getDbName());
        String tempTable = dataBase.getTempTable();
        TriggerBean[] triggers = getTriggerBeans(typeXml,appName,dataBase,table);
        IDataBase iDataBase = DBFactory.getDataBase(jdbc, Constant.DB_INTERNAL);
        iDataBase.openConnection();
        //todo
        iDataBase.createMoreSequence(appName);
        Map<String,String> map = iDataBase.createMoreTiggers(triggers,tempTable,appName);  //创建触发器,并返回触发器的名称集合
//        Map<String, String> map = iDataBase.createTriggers(triggers, tempTable);  //创建触发器,并返回触发器的名称集合
        iDataBase.closeConnection();
        return map;
    }
    ...
    /**
     * 删除触发器 若数据库连接不上,则返回删除失败
     * @param typeXml
     * @param appName
     * @param dataBase
     * @param table
     */
    private boolean removeTriggerByNameInDB(String typeXml, String appName, DataBase dataBase, Table table) throws Ex {
        boolean result = false;
        IDataBase iDataBase = null;
        try
        {
     ...
     * @throws Ex
     */
    private boolean removeSequence(String typeXml, String appName, DataBase dataBase) throws Ex{
        boolean result = false;
        IDataBase iDataBase = null;
        try{
            Jdbc jdbc = xmlOperatorDAO.getJdbcByName(typeXml,dataBase.getDbName());
            iDataBase = DBFactory.getDataBase(jdbc, Constant.DB_INTERNAL);
            iDataBase.openConnection();
            iDataBase.removeMoreSequence(appName); //根据应用删除触发器
            result = true;
        }catch (Ex ex){
            ex.printStackTrace();
        }catch (Exception e){
