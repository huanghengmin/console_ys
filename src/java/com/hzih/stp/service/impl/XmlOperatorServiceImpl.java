...
import com.hzih.stp.web.action.app.ComparatorTable;
...
public class XmlOperatorServiceImpl implements XmlOperatorService {
	 ...
	 */
	public String readInternalType(Integer start, Integer limit,String appType,Map<String,DeleteStatus> map ) throws Ex {
...
//        List<Type> list = xmlOperatorDAO.getTypes("internal",appType);
//        List<Type> internalTypes = new ArrayList<Type>();
//        for (Type type : list) {
...
//                                logger.error("源端不存在该应用"+type.getTypeName());
    ...
    public String readInternalType(Integer start, Integer limit,String appType,boolean isAllow,Map<String,DeleteStatus> map ) throws Ex {
//		Map<Integer,List<String>> internalTypes = xmlOperatorDAO.getInternalTypes(appType);
		List<Type> internalTypes = xmlOperatorDAO.getTypes("internal",appType,isAllow);

//        List<Type> list = xmlOperatorDAO.getTypes("internal",appType,isAllow);
//        List<Type> internalTypes = new ArrayList<Type>();
//        for (Type type : list) {
//            if(map.get(type.getTypeName()) != null) {
//                internalTypes.add(type);
...
//        }
	 ...
	 */
	public String readExternalType(Integer start, Integer limit,String appType, Map<String, DeleteStatus> map) throws Ex {


        List<Type> externalTypes = xmlOperatorDAO.getTypes("external",appType);
//        List<Type> list = xmlOperatorDAO.getTypes("external",appType);
//        List<Type> externalTypes = new ArrayList<Type>();
//        for (Type type : list) {
//            if(map.get(type.getTypeName()) != null) {
...
//        }
					...
					if(i==start&&index<limit){
						...
						if(sourceFile !=null && targetFile == null){
							...
							json += "{'appName':'"+type.getTypeName()+"','appDesc':'"+type.getDescription()+"',isAllow:"+type.isAllow()+",speed:"+type.getSpeed()+
								...
								"','fileListSize':'"+sourceFile.getFilelistsize()+"','packetSize':'"+sourceFile.getPacketsize()+
								"','threads':'"+sourceFile.getThreads()+"','t_protocol':'"+
								"','isTwoWay':'"+sourceFile.isIstwoway()+"','plugin':'1',deleteFlag:"+getSourceDeleteFlag(type.getTypeName())+",flag:'"+flag+"','isBackUp':'" +  sourceFile.isBackUp() +
                                "','backUpDir':'" + sourceFile.getBackUpDir() + "'},";
						}else if(sourceFile ==null && targetFile != null){
							flag = "flag_2";
							json += "{'appName':'"+type.getTypeName()+"','appDesc':'"+type.getDescription()+"',isAllow:"+type.isAllow()+",speed:"+type.getSpeed()+
                                ",channel:'"+type.getChannel()+"',channelport:'"+type.getChannelPort()+ "',status:'"+type.getStatus()+
							...
							json += "{'appName':'"+type.getTypeName()+"','appDesc':'"+type.getDescription()+ "',isAllow:"+type.isAllow()+",speed:"+type.getSpeed()+
								...
								"','fileListSize':'"+sourceFile.getFilelistsize()+"','packetSize':'"+sourceFile.getPacketsize()+
								"','threads':'"+sourceFile.getThreads()+"','t_protocol':'"+targetFile.getProtocol()+
								"','isTwoWay':'"+sourceFile.isIstwoway()+"','plugin':'0',deleteFlag:"+getSourceDeleteFlag(type.getTypeName())+",flag:'"+flag+"','isBackUp':'" +  sourceFile.isBackUp() +
                                    "','backUpDir':'" + sourceFile.getBackUpDir() + "'},";
						}						
					}
				}
	...
	public String readExternalType(Integer start, Integer limit,String appType,boolean isAllow,Map<String,DeleteStatus> map) throws Ex {
		List<Type> externalTypes = xmlOperatorDAO.getTypes("external",appType,isAllow);

//        List<Type> list = xmlOperatorDAO.getTypes("external",appType,isAllow);
//        List<Type> externalTypes = new ArrayList<Type>();
//        for (Type type : list) {
//            if(map.get(type.getTypeName()) != null) {
//                externalTypes.add(type);
...
//        }
    ...
    private boolean getSourceDeleteFlag(String appName) {
        ...
        if(deleteStatus!=null ){
            if(deleteStatus.getDeleteType()==1){
                return false;
            }
        }
        return true;
...
//                index ++;
    ...
    @Override
    public String checkJdbcNameIsNowUsed(boolean privated, String jdbcName) throws Ex {
        return xmlOperatorDAO.getAppsByJdbcName(privated, jdbcName) ;
    }
    ...
    public String[] getSysArray(String[] sysArray) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        String[] sys = xmlOperatorDAO.getSysLogClient(isPrivate);
        return StringUtils.arrayTrim(sys,sysArray);
    }

    @Override
    public String[] getSnmpArray(String[] snmpArray) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();
        String[] snmps = xmlOperatorDAO.getSNMPClient(isPrivate);
    ...
    public void sortTable(String typeXml, String tables, String appName, String dbName) throws Ex{
        if("tables_1".equals(tables)){
            //获取table列表,并按当前同步顺序排序
            List list = xmlOperatorDAO.readSourceTables(typeXml, appName,dbName);
            ComparatorTable comparator=new ComparatorTable();
            ...
            Jdbc jdbc = getJdbc(tb,dbName);
            IDataBase db = DBFactory.getDataBase(jdbc, Constant.DB_INTERNAL);
            db.openConnection();
            for(int i=0;i<total;i++){
                Table table = (Table)list.get(i);
                map = foreignKey(table.getTableName(),map,tableMap,db);
            }
            db.closeConnection();
            //更新同步顺序配置
            for(int i=0;i<total;i++){
                ...
                if(map.containsKey(table.getTableName())){
                    if(table.getSeqNumber() != map.get(table.getTableName())){
                        table.setSeqNumber(map.get(table.getTableName()).toString());
    ...
    @Override
    public List<Type> readType(String typeXml, String appType) {
        List<Type> types = null;
        try{
            if(typeXml.toLowerCase().equals("internal")){
                types = xmlOperatorDAO.getTypes("internal",appType);
            }else{
                types =  xmlOperatorDAO.getTypes("external",appType);
            }
        } catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public List<Table> readTableNames(String typeXml, String appName, String dbName) throws Ex {
        List<Table> tables = null;
        tables = (List<Table>)xmlOperatorDAO.readSourceTables(typeXml, appName,dbName);
        return tables;
    }

    public  Map<String ,Integer> foreignKey(String tableName,Map<String,Integer> map,Map<String,Table> tableMap,IDataBase db){
        if(!map.containsKey(tableName)){
            //判断该table是否存在外键
            List foreignlist = null;
            try {
                foreignlist = db.getForeignKey(tableName);
                if(foreignlist == null || foreignlist.size() == 0){
                    map.put(tableName ,map.size()+1);
                }else{
                    for(int i=0;i<foreignlist.size();i++){
                        if(tableMap.containsKey(foreignlist.get(i).toString()) && !map.containsKey(foreignlist.get(i).toString())){
                            map = foreignKey(foreignlist.get(i).toString(),map,tableMap,db);
                        }
                    }
                    map.put(tableName,map.size()+1);
                }
...
//        json += "]}";
    ...
    public String selectChangeUtils() throws Ex {
        ...
        String json = "{success:true,total:1,rows:[{restarttime:'"+iChangeUtils.getRestartTime()+"',interval:"+iChangeUtils.getInterval()+
                ...
                ",backupdir:'"+iChangeUtils.getBackupdir()+"',backupdirdelete:"+iChangeUtils.getBackupdirdelete()+
    ...
    public String readSNMP(int start, int limit) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        String[] sNMPClients = xmlOperatorDAO.getSNMPClient(isPrivate);
    ...
    public String readSysLog(int start, int limit) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        String[] sysLogClients = xmlOperatorDAO.getSysLogClient(isPrivate);
        String jsonData = null;
    ...
    public String checkSNMPClient(String snmpclient) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        String[] clients = xmlOperatorDAO.getSNMPClient(isPrivate);
    ...
    public String checkSysLogClient(String syslogclient) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        String[] clients = xmlOperatorDAO.getSysLogClient(isPrivate);
        for(int i = 0; i<clients.length;i++){
    ...
    public void saveSNMPClient(String ip) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        xmlOperatorDAO.saveSNMPClient(ip,isPrivate);
    }

    @Override
    public void saveSysLogClient(String ip) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        xmlOperatorDAO.saveSysLogClient(ip,isPrivate);
    }

    @Override
    public void deleteSNMPClient(String ip) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        xmlOperatorDAO.deleteSNMPClient(ip,isPrivate);
    }

    @Override
    public void deleteSysLogClient(String ip) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        xmlOperatorDAO.deleteSysLogClient(ip,isPrivate);
    }

    @Override
    public void updateSNMPClient(String snmpclient, String oldSNMPClient) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        xmlOperatorDAO.updateSNMPClient(snmpclient,oldSNMPClient,isPrivate);
    }

    @Override
    public void updateSysLogClient(String syslogclient, String oldSysLogClient) throws Ex {
        boolean isPrivate = StringUtils.getPrivated();//true: 目标端,false: 源端
        xmlOperatorDAO.updateSysLogClient(syslogclient,oldSysLogClient,isPrivate);
    }
    ...
    public String checkAppName(String appName) throws Ex {
		...
		if(isExitExternalS || appName.equalsIgnoreCase("audit")){
			return "该应用编号已经存在!";
	    }
        return "0000";
    }
    ...
    public SourceFile getSourceFile(String appName) throws Ex{
        return xmlOperatorDAO.getSourceFile(appName);
    }

    public TargetFile getTargetFile(String appName) throws Ex{
        return xmlOperatorDAO.getTargetFile(appName);
    }

    @Override
    public Type readExternalTypeByAppName(String appName) throws Ex {

        return xmlOperatorDAO.getExternalTypeByName(appName);
    }
