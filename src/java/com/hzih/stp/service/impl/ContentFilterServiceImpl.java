...
public class ContentFilterServiceImpl implements ContentFilterService {
    ...
    public String update(ContentFilter contentFilter) throws Exception {
        ContentFilter old = (ContentFilter) contentFilterDao.getById(ContentFilter.class,contentFilter.getId());
    ...
    @Override
    public String getAllToString() throws Exception {
        List<ContentFilter> list = contentFilterDao.findAll();
        String keyWords = "";
        int idx = 0;
        for (ContentFilter c : list){
            idx ++;
            if(idx < list.size()) {
                keyWords += c.getFilter() + ",";
            } else {
                keyWords += c.getFilter();
            }
        }
