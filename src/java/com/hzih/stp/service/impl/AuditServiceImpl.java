...
public class AuditServiceImpl implements AuditService{
     ...
     * 分页读取用户日志--并以json形式返回
     */
    public String selectManagerAudit(int pageIndex, int limit, Date startDate, Date endDate,
                                  String logLevel, String userName) throws Exception {
        PageResult pageResult = managerOperLogDao.listLogsByParams(pageIndex,limit,startDate, endDate, logLevel, userName);
        List<ManagerOperLog> managerOperLogs = pageResult.getResults();
        int total = pageResult.getAllResultsAmount();
        String json = "{success:true,total:"+ total + ",rows:[";
        for (ManagerOperLog u : managerOperLogs) {
            json +="{id:'"+u.getId()+"',userName:'"+u.getUserName()+"',level:'"+u.getLevel()+
                    "',auditModule:'"+u.getAuditModule()+"',auditInfo:'"+u.getAuditInfo()+
                    "',logTime:'"+ DateUtils.formatDate(u.getLogTime(), "yy-MM-dd HH:mm:ss")+"',flag:"+u.getFlag()+"},";
        }
        json += "]}";
        return json;
    }
...
//            json = "{success:true,total:1,rows:[{businessName:'file_001',fileName:'10000.tmp',businessType:'file',date:'2013-03-28 12:22:35'}]}";
    ...
    public String selectBusinessAudit(int pageIndex, int limit, Date startDate,
        ...
        for (BusinessLog log : list) {
            ...
            if(StringContext.EXTERNAL.equals(log.getPlugin())){
                type = xmlOperatorDAO.getExternalTypeByName(log.getBusinessName());
            }else if(StringContext.INTERNAL.equals(log.getPlugin())){
            ...
            int count = log.getAuditCount();
            int countEx = log.getAuditCountEx();
            json +="{id:"+log.getId()+",businessName:'"+log.getBusinessName()+"',level:'"+log.getLevel()+
                    ...
                    "',auditCount:'"+count+" / "+countEx+"',logTime:'"+DateUtils.formatDate(log.getLogTime(),"yy-MM-dd HH:mm:ss")+
                    "',jdbc:'"+jdbc+
