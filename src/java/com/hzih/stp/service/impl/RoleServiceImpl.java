...
public class RoleServiceImpl implements RoleService {
    ...
    public String select(int start, int limit, int accountType) throws Exception{
        ...
        List<Role> rolesAll = roleDao.findAll();
        List<Role> roles = new ArrayList<Role>();
        for (Role role : rolesAll) {
            int roleTypeCode = (int) role.getRoleType();
            if(accountType == 1) {
                if(roleTypeCode<=4){
                    roles.add(role);
                }
        ...
        for (Role role : roles) {
             if(index == start && count < limit){
                 RoleType roleType = (RoleType) roleTypeDao.getById(RoleType.class,role.getRoleType());
    ...
    public String getTypeKeyValue(int accountType) throws Exception {
        String json = "{success:true,total:";
        List<RoleType> roleTypesAll = roleTypeDao.findAll();
        List<RoleType> roleTypes = new ArrayList<RoleType>();
        for (RoleType roleType : roleTypesAll) {
            int roleTypeCode = (int) roleType.getId();
            if(accountType == 1) {
                if(roleTypeCode<=4){
        ...
        for (RoleType roleType : roleTypes) {
            json +="{value:'"+roleType.getId()+"',key:'"+roleType.getName()+"'},";
        }
        json += "]}";
        return json;
    }
    ...
    public String selectRoleType(int start, int limit, int accountType) throws Exception {
        String json = "{success:true,total:";
        List<RoleType> roleTypesAll = roleTypeDao.findAll();
        PageResult pr = roleTypeDao.listPage(start/limit+1, limit, accountType);
        List<RoleType> roleTypes = pr.getResults();
        int total = roleTypes.size();
        json += total + ",rows:[";
        for (RoleType roleType : roleTypes) {
            json +="{id:'"+roleType.getId()+"',name:'"+roleType.getName()+
                    "',permission:'"+roleType.getPermission()+"',autoUnLock:'"+roleType.getAutoUnLock()+"'},";
        }
        json += "]}";
        return json;
    }
    ...
    public RoleType updateRoleType(RoleType roleType) throws Exception {
        if(roleType.getName()==null){
            RoleType r = (RoleType) roleTypeDao.getById(RoleType.class,roleType.getId());
            roleType.setName(r.getName());
        }
    ...
    public RoleType getRoleType(long id) throws Exception {
        return (RoleType) roleTypeDao.getById(RoleType.class,id);
    }
