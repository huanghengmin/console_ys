...
public class AccountServiceImpl implements AccountService {
     ...
     * @return
     */
    public String select(int accountType, String userName, String status, int start, int limit,int unLock) {
        PageResult pageResult = accountDao.listByPage(accountType,userName,status,start/limit+1,limit);
        ...
        for (Account a : accounts) {
            ...
            json +="{id:'"+a.getId()+"',name:'"+a.getName()+"',userName:'"+a.getUserName()+
                    ...
                    "',createTime:'"+a.getCreatedTime()+"',modifiedTime:'"+a.getModifiedTime()+"',unLock:"+unLock+"},";
        }
        json += "]}";
        return json;
    }
    ...
    public String update(Account account,long[] rIds) throws Exception {
        ...
        old.setAccountType(account.getAccountType());
        if(rIds.length > 0){
    ...
    public String insert(Account account, long[] rIds) throws Exception {
        ...
        Account a = accountDao.findByNameAndPwd(account.getUserName(),account.getPassword());
        AccountSecurity accountSecurity = new AccountSecurity();
        accountSecurity.setId(a.getId());
        accountSecurity.setFilter(false);
        accountSecurity.setVirusScan(false);
        accountSecurity.setInfoLevel(String.valueOf(0));
        accountSecurityDao.create(accountSecurity);
    ...
    public String selectUserNameKeyValue(int accountType) throws Exception {
        ...
        String json = "{success:true,total:"+total + ",rows:[";
        for (Account a : accounts) {
            if(accountType == a.getAccountType()){//user
                json += "{key:'"+a.getUserName()+"',value:'"+a.getUserName()+"'},";
            }
        }
        json += "]}";
    ...
    public String selectFlag(String userName, long id) throws Exception {
        AccountSecurity accountSecurity = (AccountSecurity) accountSecurityDao.getById(id);
        String json = "{id:"+id+",isFilter:false,isVirusScan:false,infoLevel:0,dir:''}";
        if(accountSecurity!=null){
            json = "{id:"+id+",isFilter:"+accountSecurity.isFilter()+
                    ",isVirusScan:"+ accountSecurity.isVirusScan()+
                    ",appName:'"+ (accountSecurity.getAppName()==null?"":accountSecurity.getAppName())+
                    "',infoLevel:"+accountSecurity.getInfoLevel()+"}";
        }
        return json;
    }
    ...
    public String updateSecurity(AccountSecurity accountSecurity) throws Exception {
        AccountSecurity as = (AccountSecurity) accountSecurityDao.getById(accountSecurity.getId());
        if(as == null) {
            accountSecurityDao.create(accountSecurity);
        } else {
            accountSecurityDao.update(accountSecurity);
        }
        return "<font color=\"green\">设置成功,点击[确定]返回列表!</font>";
    }

    @Override
    public AccountSecurity selectAccountSecurity(Long id) throws Exception {
        AccountSecurity as = (AccountSecurity) accountSecurityDao.getById(id);
        return as;
    }

    @Override
    public void updateStatus(long id, String status) throws Exception {
        Account old = (Account) accountDao.getById(id);
    ...
    public void setAccountSecurityDao(AccountSecurityDao accountSecurityDao) {
