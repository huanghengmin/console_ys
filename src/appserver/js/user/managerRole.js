...
Ext.onReady(function() {
    ...
    var proxy = new Ext.data.HttpProxy({
        url:"../../RoleAction_select.action?accountType=1"
    });
...
var storeType = new Ext.data.Store({
    url:"../../RoleAction_readTypeKeyValue.action?accountType=1",
    reader:readerType
});
...
function roleInsert(formPanel,grid,store,win,ids){
    if (formPanel.form.isValid()) {
        formPanel.getForm().submit({
            url :'../../RoleAction_insert.action?accountType=1',
            method :'POST',
            params:{pIds:ids},
            waitTitle :'系统提示',
            waitMsg :'正在保存,请稍后...',
                ...
                items:[{
                    ...
                    items:[{
                        ...
                        items:[{
                            ...
                            fieldLabel:"角色类型",
                            hiddenName:'role.roleType',value:item.data.roleType,
                            mode:'local',
                            editable : false,
                            typeAhead:true,
                            forceSelection: true,
...
function roleUpdate(formPanel,grid,store,win,ids){
    if (formPanel.form.isValid()) {
        formPanel.getForm().submit({
            url :'../../RoleAction_update.action?accountType=1',
            method :'POST',
            params:{pIds:ids},
            waitTitle :'系统提示',
            waitMsg :'正在修改,请稍后...',
