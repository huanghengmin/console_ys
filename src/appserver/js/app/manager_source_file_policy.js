...
Ext.onReady(function(){
    ...
    var external_record = new Ext.data.Record.create([
        ...
        {name:'isBackUp', mapping:'isBackUp'},
        {name:'backUpDir',mapping:'backUpDir'}
    ]);
...
//        external_boxM,
        external_rowNumber,
        {header:"策略编号",		dataIndex:"appName",			align:'center'},
        {header:"策略名",			dataIndex:"appDesc",			align:'center'},
...
//        {header:'策略类型',		dataIndex:'appType',			align:'center',width:50},
...
//    }
    ...
    var external_grid_panel= new Ext.grid.GridPanel({
        ...
        title:'<center><font size="4">前置机</font></center>',
        animCollapse:true,
        height:setHeight(),
        width:setWidth(),
                        ...
                        success:function(response,option){
                            ...
                            if(msg=='true'){
                                ...
                                if(description==null||description==''||description=='null'){
                                    message = "存档当前配置文件？<br><font color='#808080'>可以输入描述信息用于说明存档后的配置(可选)!</font>";
                                } else {
                                    message = "当前配置文件描述信息:<font color='green'>"+description+"</font><br>存档当前配置文件？<br><font color='#808080'>可以输入描述信息用于说明存档后的配置(可选)!</font>";
                                }
                                Ext.MessageBox.show({
                                    title:'信息',
                                    msg:message,
...
function show_protocol(value){
	if(value == 'ftp'){
		...
		return '共享文件传输协议(SMB)';
    }else if(value=='smbdown'){
		return '共享文件传输协议(SMB备份)';
...
function external_showURL_flag(value,p,r){
    var deleteType;
    if(r.get('deleteFlag')) {
        deleteType = "<a href='javascript:;' style='color: green;' onclick='external_delete_file_row();'>删除策略</a>";
    } else {
    ...
    var detail = "<a href='javascript:;' style='color: green;' onclick='external_detail_file_win();'>详细</a>&nbsp;&nbsp;&nbsp;&nbsp;";
    var updateWin = "&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:;' style='color: green;' onclick='external_update_file_app_win();'>修改策略</a>&nbsp;&nbsp;&nbsp;&nbsp;";
    if(value=='flag_0'){
        return detail + status + deleteType + updateWin + "<a href='javascript:;' style='color: green;' onclick='external_update_file_source_win();'>修改源数据</a>";
    }else if(value=='flag_1'){
        return detail + status + deleteType + updateWin + "<a href='javascript:;' style='color: green;' onclick='external_update_file_source_win();'>修改源数据</a>";
    }else if(value=='flag_2'){
        return detail + status + deleteType + updateWin + "<font color='gray'>修改源数据</font>";
...
var dataProtocol = [
    ['ftp','FTP文件传输协议'],
//    ['ftpdown','FTP文件传输协议(备份)'],
    ['smb','共享文件传输协议(SMB)']
//    ['smbdown','共享文件传输协议(SMB备份)']
//    ['ftps','FTPS文件传输协议'],
//    ['WebDAV','WebDAV文件传输协议']
];
...
function external_insert_file_win(grid,store){
	...
	var storeChannel = new Ext.data.Store({
        ...
        listeners : {
			load : function(){
				var value = Ext.getCmp('external.combo.channel.info').getValue();
				Ext.getCmp('external.combo.channel.info').setValue(value);
			}
		}
	});
	storeChannel.load();

    var resourceRecord = new Ext.data.Record.create([
		{name:'value',mapping:'value'},
		{name:'key',mapping:'key'}
	]);
	var resourceReader = new Ext.data.JsonReader({ totalProperty:'total',root:"rows"},resourceRecord);
	var storeResource = new Ext.data.Store({
		url:'../../AppTypeAction_readPolicyDirKeyValue.action',
    ...
    //策略属性配置
     var form1 = new Ext.form.FormPanel({
        ...
        items:[{
            ...
            regex:/^([0-9]|[a-z]|[A-Z])\w{1,29}$/,
            regexText:'请输入2--30个只能包含字母、数字和下划线且不能以下划线开头的字符',
            emptyText:'请输入2--30个只能包含字母、数字和下划线且不能以下划线开头的字符',
            listeners:{
                blur:function(){
            ...
            fieldLabel:"通道",
            hiddenName:'typeBase.channel',value:1,
            xtype:'combo',
            mode:'local',
            emptyText :'--请选择--',
            editable : false,
            ...
            regex:/^(7700|7[0-6][0-9][02468]|6[7-9][0-9][02468])$/,
            regexText:'这个不是端口类型6700~7700的偶数端口',
            emptyText:'请输入端口6700~7700之间的偶数端口',
            listeners:{
                blur:function(){
        ...
        items:[{
            ...
            listeners:{
                ...
                blur:function(){
                    ...
                    if(addr.indexOf("127.0.0.1") > -1){
                        var protocol = Ext.getCmp('protocol.info').getValue();
                        if(protocol == 'ftp'){
                            Ext.getCmp("dir.info").setValue('/data/ftpsource/' + Ext.getCmp('appName.external.info').getValue());
                            Ext.getCmp("internal.backUpDir.info").setValue("/data/" + Ext.getCmp('appName.external.info').getValue());
                        }else if(protocol == 'smb'){
                            Ext.getCmp("dir.info").setValue('/data/smbsource/' + Ext.getCmp('appName.external.info').getValue());
                            Ext.getCmp("internal.backUpDir.info").setValue("/data/" + Ext.getCmp('appName.external.info').getValue());
                        }
                    }
                }
            ...
            regexText:'密码是1-16位(可以包含A-Za-z0-9_!#$%^&*.~),或者为空',
            listeners:{
                focus:function(){
                    Ext.getCmp('card-finish').hide();
                    Ext.getCmp('card-test').show();
...
//            defaultType: 'radio',
//            items: [
//                { width:50, boxLabel: '是', name: 'typeFile.BackUp', inputValue: true,  checked: true },
//                { width:50, boxLabel: '否', name: 'typeFile.BackUp', inputValue: false }
//            ],
//            listeners:{
//                change : function(group,ck){
//                    if(ck.inputValue==false){
//                        Ext.getCmp('internal.backUpDir.info').reset();
//                        Ext.getCmp('internal.backUpDir.info').disable();
//                    }else if(ck.inputValue==true){
//                        Ext.getCmp('internal.backUpDir.info').enable();
//                        Ext.getCmp('internal.backUpDir.info').reset();
//                    }
//                }
//            }
            xtype:'hidden',
            name: 'typeFile.BackUp',
            value: false
        },{
            id:'internal.backUpDir.info',
//            fieldLabel:"备份目录",
//            xtype:'textfield',
//            name:'typeFile.backUpDir',
//            regex:/^([\/].*)*$/,
//            regexText:'这个不是目录',
//            emptyText:'请输入不同于源文件目录的目录'
            fieldLabel:"资源目录",
        	xtype:'combo',
        	mode:'local',
        	emptyText :'--请选择--',
    ...
    var card = new Ext.Panel({
        ...
        bbar:['->',{
            ...
            text:'测试',
            handler:function(){
                if(Ext.getCmp('add.serverAddress.info').getValue().indexOf("127.0.0.1") > -1){
                    Ext.getCmp('card-test').hide();
                    Ext.getCmp('card-finish').show();

                }else if (form1.form.isValid()&&form2.form.isValid()) {
...
function external_delete_file_row(){
    ...
    if(selModel.hasSelection()){
        ...
        Ext.each(selections,function(item){
            ...
            Ext.MessageBox.show({
                title:'信息',
                msg:'<font color="green">确定要删除'+appName+'策略？</font>',
                ...
                fn:function(e){
                    if(e == 'ok'){
                        ...
                        myMask.show();
                        Ext.Ajax.request({
                            url : '../../FileTypeAction_deletePolicy.action',
...
function external_update_file_source_win(){
	...
	var storeResource = new Ext.data.Store({
        ...
        listeners : {
			load : function(){
				var value = Ext.getCmp('update.backUpDir.info').getValue();
				Ext.getCmp('update.backUpDir.info').setValue(value);
			}
		}
	});
	storeResource.load();
                ...
                items:[{
                    ...
                    listeners:{
                        ...
                        blur:function(){
                            var addr = this.getValue();
                            if(addr.indexOf("127.0.0.1") > -1){
                                var protocol = Ext.getCmp('update.protocol.info').getValue();
                                if(protocol == 'ftp'){
                                    Ext.getCmp("update.dir.info").setValue('/data/ftpsource/' + item.data.appName);
                                    Ext.getCmp("update.backUpDir.info").setValue("/data/" + item.data.appName);
                                }else if(protocol == 'smb'){
                                    Ext.getCmp("update.dir.info").setValue('/data/smbsource/' + item.data.appName);
                                    Ext.getCmp("update.backUpDir.info").setValue("/data/" + item.data.appName);
                                }
                            }
                        }
...
//                    defaultType: 'radio',
//                    items: [
//                        { width:50, boxLabel: '是', name: 'typeFile.BackUp', inputValue: true,  checked: backupT },
//                        { width:50, boxLabel: '否', name: 'typeFile.BackUp', inputValue: false , checked:backupF }
//                    ],
//                    listeners:{
//                        change : function(group,ck){
//                            if(ck.inputValue==false){
//                                Ext.getCmp('update.backUpDir.info').reset();
//                                Ext.getCmp('update.backUpDir.info').disable();
//                            }else if(ck.inputValue==true){
//                                Ext.getCmp('update.backUpDir.info').enable();
//                                Ext.getCmp('update.backUpDir.info').reset();
//                            }
//                        }
//                    }
...
//                    emptyText:'请输入不同于源文件目录的目录' ,
                    ...
                    items:[{
                        id:'external.filterTypes.info',
                        fieldLabel:"文件类型",
                        hiddenName:'typeFile.filterTypes',
                        name:'typeFile.filterTypes',
...
//                }
        ...
        bbar:[
            ...
            new Ext.Button({
                ...
                text:'测试',
                handler : function() {
                    if(Ext.getCmp('update.serverAddress.info').getValue().indexOf("127.0.0.1") > -1){
    ...
    if(selModel.hasSelection()){
        ...
        Ext.each(selections,function(item){
            formPanel = new Ext.form.FormPanel({
                ...
                items:[{fieldLabel:"策略编号",value:item.data.appName},
                    {fieldLabel:"策略名",value:item.data.appDesc},
                    {fieldLabel:"策略源类型",value:'数据源'},
                    {fieldLabel:'策略类型',value:'文件同步'},
                    ...
                    {fieldLabel:'自动采集频率(毫秒)',value:item.data.interval},
                    {fieldLabel:'采集后删除文件',value:show_is(item.data.deleteFile)},
