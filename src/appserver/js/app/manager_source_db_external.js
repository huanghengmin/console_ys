...
function external_insert_db_win(grid,store){
    ...
    var form1 = new Ext.form.FormPanel({
        ...
        items:[{
            ...
            regex:/^([0-9]|[a-z]|[A-Z])\w{1,19}$/,
            regexText:'请输入2--20个只能包含字母、数字和下划线且不能以下划线开头的字符',
            emptyText:'请输入2--20个只能包含字母、数字和下划线且不能以下划线开头的字符',
            listeners:{
                blur:function(){
        ...
        items:[{
            ...
            listeners:{
                blur:function(){
                    ...
                    //触发同步只允许输入一个临时表
                    var op = Ext.getCmp("tbfs.combo.info").getValue();
                    if(op.indexOf("trigger") > -1 && "trigger".indexOf(op) > -1){
                        //触发同步
                        if(tempTable.indexOf(",") > -1){
                            Ext.MessageBox.show({
                                title:'信息',
                                msg:'<font color="red">触发同步仅允许使用一张临时表!</font>',
                    ...
                    listeners:{
                        blur:function(){
                            ...
                            if(tempTable.length>0){
                                //触发同步只允许输入一个临时表
                                var op = Ext.getCmp("update.tbfs.combo.info").getValue();
                                if(op.indexOf("trigger") > -1 && "trigger".indexOf(op) > -1){
                                    //触发同步
                                    if(tempTable.indexOf(",") > -1){
                                        Ext.MessageBox.show({
                                        ...
                                        Ext.getCmp('external.update.fzbm.textfield.info').setValue(item.data.tempTable);
                                        tempTable = item.data.tempTable;
                                    }
                                }
                                if(item.data.tableTable == null || item.data.tableTable.length == 0 || (!tempTable.indexOf(item.data.tempTable) > -1 && !item.data.tempTable(tempTable) > -1)){
                                    var myMask = new Ext.LoadMask(Ext.getBody(),{
                                        msg : '正在校验,请稍后...',
                                        removeMask : true
                                    });
                                    myMask.show();
                                    var _dbName = Ext.getCmp('update.dbName.info').getValue();
                                    Ext.getCmp('external.update.data.win.info').setDisabled(true);
                                    Ext.Ajax.request({
                                        url:'../../DBTypeAction_checkUpdateTempTable.action',
                                        params:{tempTable:tempTable,typeXml:'external',dbName:_dbName,oldTempTable:item.data.tempTable},
                                        method:'POST',
                                        success:function(action){
                                            var json = Ext.decode(action.responseText);
                                            myMask.hide();
        ...
        bbar:[
            ...
            new Ext.Button ({
                ...
                handler : function() {
                    ...
                    var tempTable = Ext.getCmp('external.update.fzbm.textfield.info').getValue();
                    if (formPanel.form.isValid()) {
                        Ext.MessageBox.show({
                    		title:'信息',
                            width:200,
			                                ...
			                                Ext.MessageBox.show({
			                                    ...
			                                    fn:function(e){
			                                        if(e=='ok'){
                                                        ...
                                                        store.reload();
                                                        win.close();
                                                        external_detail_db_attribute_source(appName,dbName,operation,tempTable);
                                                    }
			                                    }
			                                });
			                            }
 ...
 * 操作数据库
 */
function external_operate_db(appName,dbName){
    Ext.MessageBox.show({
        title:'信息',
        ...
        fn:function(e){
            if(e == 'ok'){
                ...
                Ext.Ajax.request({
                    ...
                    params :{appName : appName,typeXml:'external',dbName:dbName},
                    method:'POST',
                    timeout:60*1000*3,
                    success : function(r,o){
                        var respText = Ext.util.JSON.decode(r.responseText);
 ...
 * 数据源 表集合 查找
 */
function external_detail_db_attribute_source(appName,dbName,operation,tempTable){
...
//        enableDragDrop: true,
        ...
        tbar:[
            new Ext.Button({
                ...
                iconCls:'add',
                handler:function(){
                    external_insert_read_db_source_tables(grid,store,appName,dbName,operation,tempTable);
                }
            }),
            {xtype:"tbseparator"},
            new Ext.Button({
                id:'btnUpdate.db.external.source.table.info',
                text:'批量修改',
                iconCls:'update',
                handler:function(){
                    external_update_read_db_source_tables(grid,store,appName,dbName,operation);
                }
            }),
            {xtype:"tbseparator"},
			new Ext.Button ({
 ...
 * 数据源 表集合 增加 -- 选择 源表
 */
function external_insert_read_db_source_tables(grid,store,appName,dbName,operation,tempTable){
//    var external_grid = Ext.getCmp('grid.db.external.info');
//    var external_store = external_grid.getStore();
//    var selModel = external_grid.getSelectionModel();
//    var appName;
...
//        enableDragDrop: true,
        ...
        tbar:[{
            ...
            iconCls:'add',
            handler:function(){
                external_insert_more_db_source_tables(grid,appName,dbName,operation,tempTable);
            }
//        },{xtype:"tbseparator"},{
        }],
        bbar:page_toolbar
    ...
    function external_showURL_tableAttribute_flag(value,p,r){
        var tableName = r.get('tableName');
 ...
 * @param tempTable
 */
function external_update_read_db_source_tables(grid,store,appName,dbName,operation){
    //todo
    var store = grid.getStore();
    var selModel = grid.getSelectionModel();
    ...
    if(count==0){
        ...
        var win = new Ext.Window({
            ...
            items: [formPanel],
            bbar:['->',{
                id:'more.insert.win.info',
                text:'保存',
                handler:function(){
                    var monitorinsert = false;
                    var monitorupdate = false;
                    var monitordelete = false;
                    if(operation.indexOf('trigger')) {
                        monitorinsert = Ext.getCmp('update.insert.info').getValue();
                        monitorupdate = Ext.getCmp('update.update.info').getValue();
                        monitordelete = Ext.getCmp('update.delete.info').getValue();
                    }
                    ...
                    Ext.MessageBox.show({
                        title:'信息',
                        msg:'<font color="green">确定要修改所选记录对应的属性？</font>',
                        animEl:'more.update.win.info',
                        width:260,
                        buttons:{'ok':'确定','no':'取消'},
                        icon:Ext.MessageBox.INFO,
                        closable:false,
                        fn:function(e){
                            if(e == 'ok'){
                                if (formPanel.form.isValid()) {
                                    formPanel.getForm().submit({
                                        url : '../../DBSourceTableAction_insertMoreSourceTableField.action',
                                        params :{
                                            tableNames:tableNames,type:'source', operate:'update',
                                            monitordelete:monitordelete,monitorinsert:monitorinsert,monitorupdate:monitorupdate
                                        },
                                        method:'POST',
                                        timeout:60*1000*3,
                                        waitTitle :'系统提示',
            ...
            listeners:{
                show:function(){
                    ...
                    }else if(operation.indexOf('addtrigger') > -1){
                        Ext.getCmp('update.update.info').disable();
                        Ext.getCmp('update.delete.info').disable();
                    }
                }
            }
        }).show();
 ...
 * @param tempTable
 */
function external_insert_more_db_source_tables(grid,appName,dbName,operation,tempTable){
    var store = grid.getStore();
    var selModel = grid.getSelectionModel();
    var count = selModel.getCount();
    if(count==0){
        ...
        var win = new Ext.Window({
            ...
            bbar:['->',{
                ...
                handler:function(){
                    ...
                    var monitorupdate = false;
                    var monitordelete = false;
                    if(operation.indexOf('trigger') > -1) {
                        monitorinsert = Ext.getCmp('save.insert.info').getValue();
                        monitorupdate = Ext.getCmp('save.update.info').getValue();
                        monitordelete = Ext.getCmp('save.delete.info').getValue();
                    }
                    var tableNames = new Array();
                    var record = grid.getSelectionModel().getSelections();
                    for(var i = 0; i < record.length; i++){
                    ...
                    Ext.MessageBox.show({
                        title:'信息',
                        msg:'<font color="green">确定要新增所选记录对应的属性？</font>',
                        animEl:'more.insert.win.info',
                        width:260,
                        buttons:{'ok':'确定','no':'取消'},
                        icon:Ext.MessageBox.INFO,
                            ...
                            if(e == 'ok'){
                                if (formPanel.form.isValid()) {
                                    formPanel.getForm().submit({
                                        url : '../../DBSourceTableAction_insertMoreSourceTableField.action',
                                        params :{
                                            tableNames:tableNames,type:'source',operate:'insert',
                                            monitordelete:monitordelete,monitorinsert:monitorinsert,monitorupdate:monitorupdate
                                        },
                                        method:'POST',
                                        waitTitle :'系统提示',
            ...
            listeners:{
                show:function(){
                    if(operation=='entirely'||operation=='timesync'||operation=='delete'||operation=='flag'){
                        ...
                        Ext.getCmp('save.delete.info').setValue(false) ;
                        Ext.getCmp('save.update.info').setValue(false) ;
                        Ext.getCmp('save.delete.info').disable();
                        Ext.getCmp('save.update.info').disable();
                    }
                }
            }
        }).show();
...
//        enableDragDrop: true,
    ...
    var isCheck_add = false,isCheck_update=false,isCheck_delete=false;
        ...
        listeners:{
            show:function(){
                if(operation=='entirely'||operation=='timesync'||operation=='delete'||operation=='flag'){
                    ...
                    Ext.getCmp('external.form.table.monitorinsert.info').setValue(true);
                    Ext.getCmp('external.form.table.monitorupdate.info').disable();
                    Ext.getCmp('external.form.table.monitordelete.info').disable();
                }
            }
        }
    }).show();
