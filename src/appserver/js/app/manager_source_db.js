...
var synchronizeMethod = [['entirely','全表同步'],['trigger','触发同步'],['delete','删除同步'],['flag','标记同步'],['timesync','时间标记同步'],['addtrigger','增加触发同步']];
...
function external_showURL_flag(value,p,r){
    ...
    if(r.get('operateDB')){
        var appName = r.get('appName');
        var dbName = r.get('dbName');
        operateDB = "<a href='javascript:;' onclick='external_operate_db(\""+appName+"\",\""+dbName+"\");' style='color: green;'>操作数据库</a>";
    } else {
