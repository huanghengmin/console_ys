...
function internal_insert_read_db_target_tables(){
    ...
    var toolbar = new Ext.Toolbar({
        ...
        items : [{
            ...
            iconCls:'add',
            handler:function(){
                internal_insert_more_db_target_tables(grid,appName,target_dbName_store,sourceDBName);
            }
        },{xtype:"tbseparator"},
        '&nbsp;&nbsp;&nbsp;&nbsp;设置一个默认的目标数据源:&nbsp;&nbsp;&nbsp;&nbsp;',{
            xtype : 'combo',
...
function internal_insert_more_db_target_tables(grid,appName,target_dbName_store,sourceDBName){
    var store = grid.getStore();
    var selModel = grid.getSelectionModel();
    var count = selModel.getCount();
    if(count==0){
        ...
        var formPanel = new Ext.form.FormPanel({
            ...
            items:[{
                xtype : 'combo',
                fieldLabel:'目标端数据源',
                hiddenName:'typeTable.targetDBName',
                store : target_dbName_store,
                valueField : 'value',
                displayField : 'key',
                mode : 'local',
            ...
            bbar:['->',{
                ...
                handler:function(){
                    ...
                    for(var i = 0; i < record.length; i++){
                        tableNames[i] = record[i].get('sourceTableName');
                    }
                    Ext.MessageBox.show({
                        title:'信息',
                        ...
                        fn:function(e){
                            if(e == 'ok'){
                                if (formPanel.form.isValid()) {
                                    ...
                                    formPanel.getForm().submit({
                                        url : '../../DBTargetTableAction_insertMoreTargetTableField.action',
                                        params :{
                                            tableNames:tableNames,type:'target',sourceDBName:sourceDBName,
                                            ...
                                            Ext.MessageBox.show({
                                                ...
                                                fn:function(e){
                                                    if(e=='ok'){
                                                        grid.render();
                                                        store.reload();
                                                        Ext.getCmp('grid.table.query.target.internal.info').render();
