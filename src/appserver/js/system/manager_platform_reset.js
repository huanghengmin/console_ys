...
Ext.onReady(function(){
    ...
    var initForm = new Ext.form.FormPanel({
        ...
        items:[{
        	...
        	fieldLabel:"SSH",
        	html:"<a href='javascript:;' onclick='init_ssh();'style='color: green;'>恢复</a>"
        }]
    });
    ...
    Ext.MessageBox.show({
        ...
        fn:function(e){
            if(e=='ok'){
				...
				myMask.show();
				Ext.Ajax.request({
                    url : '../../ManagerDataBaseAction_initSsh.action',
                    method:'POST',
                    success : function(form, action) {
                        myMask.hide();
                        Ext.Msg.alert('提示信息',
                            '启动SSH<font color="green">成功</font>!');
                    },
                    failure : function(form, action) {
                        myMask.hide();
                        Ext.Msg.alert('提示信息',
                            '启动SSH<font color="red">失败</font>!');
                    }
                });
            }
        }
